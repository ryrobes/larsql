...
including soundings, reforges, retries, and nested cascades.
...
class ExecutionTreeBuilder:
    ...
    def _build_phase_data(self, phase_node: ExecutionNode, nodes_map: Dict[str, ExecutionNode]) -> Dict:
        ...
        if sounding_attempts:
            ...
            soundings.sort(key=lambda s: s.get('index') or 0)

        # Check for evaluator entry for soundings
        evaluator_content = ""
        ...
        if qm_entries:
            quartermaster_entry = {
                ...
                'selected_tackle': qm_entries[0].metadata.get('selected_tackle', []),
        ...
        return {
            ...
            'evaluator_content': evaluator_content if soundings else None,
...
def build_react_flow_nodes(tree: Dict) -> Dict:
    ...
    for phase_idx, phase in enumerate(phases):
        ...
        if phase_type == 'soundings' and soundings:
            ...
            # Create individual sounding attempt nodes
            attempt_ids = []
            ...
            if quartermaster:
                ...
                selected_tackle = quartermaster.get('selected_tackle', [])
                if selected_tackle:
                ...
                nodes.append({
                    ...
                    'data': {
                        'label': qm_label,
                        'selected_tackle': selected_tackle,
