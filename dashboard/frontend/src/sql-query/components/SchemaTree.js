...
function SchemaNode({ schema, connectionName }) {
  const { toggleNodeExpanded, isNodeExpanded } = useSqlQueryStore();
...
function TableNode({ table, connectionName, schemaName }) {
  const { toggleNodeExpanded, isNodeExpanded, updateTab, activeTabId } = useSqlQueryStore();
...
function SchemaTree() {
  const { connections, connectionsLoading, connectionsError } = useSqlQueryStore();
