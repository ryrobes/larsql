 ...
 *
 * Adapted from Playground's CascadeBrowser for Studio use.
 */
function CascadeBrowserModal({ isOpen, onClose, onLoad }) {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5001/api/playground/browse')
      .then(res => res.json())
      .then(data => {
        if (data.error) {
          setError(data.error);
  ...
  return (
    <div className="studio-browser-overlay" onClick={onClose}>
      <div className="studio-browser-modal" onClick={e => e.stopPropagation()}>
        {/* Header */}
...
export default CascadeBrowserModal;
