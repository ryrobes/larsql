 ...
 *
 * Features:
 * - Subtle floating button in bottom-right corner
 * - Tracks currently focused input/textarea
 * - Records audio and transcribes via API
 * - Inserts transcript into the last focused input
...
function GlobalVoiceInput() {
  ...
  useEffect(() => {
    ...
    const handleBlur = (e) => {
      ...
      if (relatedTarget?.closest('.global-voice-input')) return;
  ...
  return (
    <div className={`global-voice-input ${status} ${isExpanded ? 'expanded' : ''}`}>
      {/* Main button */}
      <button
        ...
        <div className="info-panel">
          {status === 'recording' && (
            <>
              ...
              <span className="hint">Click to stop</span>
            </>
          )}
