# RVBBIT Request Decision System
...
`request_decision` is the **canonical** human-in-the-loop (HITL) tool in RVBBIT. It provides a universal checkpoint system that pauses cascade execution to request human input, then continues with the response.
...
**Location**: `rvbbit/traits/human.py:5-141`
...
**Location**: `rvbbit/traits/human.py:171-516`
...
**Location**: `rvbbit/traits/human.py:614-1060`
...
rvbbit run examples/hitl_choice_demo.yaml --input '{}' --session test_simple

# Structured decision (request_decision with options)
rvbbit run examples/tool_based_decisions.yaml --input '{"task": "Design a feature"}' --session test_structured

# HTMX Plotly chart (request_decision with html)
rvbbit run examples/htmx_plotly_demo.yaml --input '{"analysis_topic": "Sales trends"}' --session test_plotly

# All UI types demo
rvbbit run examples/hitl_generative_ui_demo.yaml --input '{}' --session test_all_types
```
...
- **Location**: `rvbbit/checkpoints.py:537-546`
- **Fix**:
  ```python
...
**Verify Fix Applied**:
```bash
# In rvbbit/checkpoints.py, check wait_for_response() method
grep -A 10 "Force reload from database" rvbbit/rvbbit/checkpoints.py
```
...
**The `request_decision` tool is now the universal HITL primitive in RVBBIT.**
