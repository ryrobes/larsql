"""
LARS - Declarative Agent Framework for Python
...
def _register_all_skills():
    ...
    from .validators import validate_parse_expression
    ...
    register_skill("lars_data", lars_data)
    ...
    if name in ("get_model_registry", "local_model_tool", "auto_device", "get_device_info"):
        ...
        if local_models_available():
            ...
            if name == "get_model_registry":
                def stub():
                    raise ImportError("Local models not available. Install with: pip install lars[local-models]")
                ...
                def stub():
                    return {"error": "Local models not available. Install with: pip install lars[local-models]"}
                _LOADED_ATTRS[name] = stub
                return stub

    raise AttributeError(f"module 'lars' has no attribute '{name}'")
