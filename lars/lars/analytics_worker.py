...
# Build a lookup table of cascade_id -> internal flag by scanning all cascade files
# This is done once at module load and cached
_INTERNAL_CASCADE_LOOKUP: Dict[str, bool] = {}

def _build_internal_cascade_lookup() -> Dict[str, bool]:
    """
    Scan all cascade directories and build a lookup table: cascade_id -> internal flag.

    This avoids relying on any naming convention between cascade_id and filename.
    """
    import yaml
    import glob
    ...
    for scan_dir in scan_dirs:
        ...
        # Find all YAML files (but not subdirectories for cascades/ to avoid duplicates)
        if scan_dir.endswith('cascades') and not scan_dir.endswith('semantic_sql'):
            # Only scan top-level cascades, not subdirs (semantic_sql is scanned separately)
            pattern = os.path.join(scan_dir, '*.yaml')
        else:
            pattern = os.path.join(scan_dir, '*.yaml')
        ...
        for filepath in glob.glob(pattern):
            try:
                ...
                if config and isinstance(config, dict):
                    ...
                    if cascade_id:
                        lookup[cascade_id] = internal

            except Exception as e:
                logger.debug(f"Could not parse cascade {filepath}: {e}")
                continue

    logger.debug(f"Built internal cascade lookup: {len(lookup)} cascades, {sum(lookup.values())} internal")
    return lookup

# Build lookup at module load
_INTERNAL_CASCADE_LOOKUP = _build_internal_cascade_lookup()
...
def is_internal_cascade_by_id(cascade_id: str) -> bool:
    ...
    Uses pre-built lookup table (fast, O(1)).
    """
    return _INTERNAL_CASCADE_LOOKUP.get(cascade_id, False)
    ...
    try:
        ...
        try:
            ...
            if CONFIDENCE_ASSESSMENT_ENABLED and not is_internal:
                ...
                thread.start()
                logger.info(f"[Analytics] [OK] Queued confidence assessment for {session_id} ({session_cascade_id})")
            else:
        ...
        import os
        cells_with_context = cascade_context_metrics.get('cells_with_context', 0)
        relevance_enabled = os.getenv('LARS_ENABLE_RELEVANCE_ANALYSIS', 'true').lower() == 'true'
...
def _fetch_session_data(session_id: str, db) -> Optional[Dict]:
    ...
    try:
        ...
        return {
            ...
            'caller_id': session_info.get('caller_id', ''),  # [OK] Include caller tracking
...
def _analyze_cells(session_id: str, db, cascade_id: str, genus_hash: str,
    ...
    try:
        ...
        for idx, cell in enumerate(cell_results):
            ...
            # Analyze context attribution (LARS's unique capability!)
            ...
            cell_row = {
                ...
                # Context attribution (LARS's unique insight!)
...
def _analyze_context_attribution(session_id: str, cell_name: str, db) -> Dict:
    ...
    Uses LARS's unique context_hashes tracking to calculate token/cost
...
def _analyze_context_relevance(session_id: str, cascade_id: str, cell_name: str,
    ...
    try:
        ...
        logger.info(f"[Relevance] [OK] {cell_name}: scored {len(relevance_scores)} messages, updated {actual_count} rows, cost=${analysis_cost:.6f}, session={analysis_session_id}")
...
def _create_context_breakdown(session_id: str, cell_name: str, cell_index: int,
    ...
    try:
        ...
        if breakdown_rows:
            ...
            # Optional: Run relevance analysis (enabled by default, opt-out with LARS_ENABLE_RELEVANCE_ANALYSIS=false)
            if os.getenv('LARS_ENABLE_RELEVANCE_ANALYSIS', 'true').lower() == 'true':
