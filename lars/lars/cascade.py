...
class BrowserConfig(BaseModel):
    ...
    When a cell has browser config, LARS will:
...
class RagConfig(BaseModel):
    ...
    Uses the standard LARS provider config for embeddings.
    Set LARS_DEFAULT_EMBED_MODEL to override the default embedding model.
    """
    directory: str
    ...
    model: Optional[str] = None  # Embedding model override (defaults to LARS_DEFAULT_EMBED_MODEL)
...
class CellConfig(BaseModel):
    ...
    # When set, LARS spawns a dedicated Rabbitize browser subprocess for this cell
    ...
    use_training: bool = True                     # Auto-inject verified examples (opt-out with false)
    ...
    training_verified_only: bool = True           # Only use human-verified examples
...
class CascadeConfig(BaseModel):
    ...
    # DB files stored in $LARS_ROOT/research_dbs/{name}.duckdb
    ...
    # Use `lars triggers export` to generate external scheduler configs
