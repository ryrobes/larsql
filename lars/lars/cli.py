...
# This keeps `lars --help` and other lightweight commands fast (~0.1s vs ~2.5s)
...
def main():
    ...
    render_parser.add_argument("image", help="Path to image file to render (e.g., dashboard/frontend/public/lars-spicy.png)")
    ...
    db_cleanup_parser = db_subparsers.add_parser(
        'cleanup-results',
        help='Drop expired query result tables from lars_results database'
    )
    ...
    # Supports both: `lars ssql "SELECT..."` (direct query) and `lars ssql test` (subcommand)
    ...
    sql_server_parser = sql_subparsers.add_parser(
        ...
        aliases=['serve']  # Allow 'lars sql serve'
    )
    ...
    sql_semantic_parser = sql_subparsers.add_parser(
        ...
        aliases=['sem', 'ssql']  # Allow 'lars sql sem' or 'lars sql ssql'
    )
    ...
    triggers_export_parser.add_argument('--image', default='lars:latest', help='Docker image (for k8s format)')
    ...
    serve_studio_parser = serve_subparsers.add_parser(
        'studio',
        help='Start the LARS Studio web UI backend'
    )
    ...
    browser_batch_parser.add_argument(
        '--client-id',
        default='lars',
        help='Client ID for session (default: lars)'
    )
    browser_batch_parser.add_argument(
        '--test-id',
        default='batch',
    ...
    # Init command - Initialize a new LARS workspace
    init_parser = subparsers.add_parser(
        'init',
        help='Initialize a new LARS workspace with starter files'
    )
    ...
    if args.command is None:
        if len(sys.argv) > 1 and (sys.argv[1].endswith('.json') or sys.argv[1].endswith('.yaml')):
            # Legacy mode: lars config.json --input {...}
            args.command = 'run'
    ...
    if args.command == 'run':
        ...
        # Note: `lars ssql "SELECT..."` is preprocessed to `lars ssql query "SELECT..."`
        if args.ssql_command in ('query', 'q', 'run'):
            ...
            print("Usage: lars ssql \"SELECT...\" or lars ssql <subcommand>")
            print("\nSubcommands: query, test, list")
            print("\nExamples:")
            print("  lars ssql \"SELECT normalize('ACME Corp', 'company')\"")
            print("  lars ssql \"SELECT * FROM data WHERE col MEANS 'tech'\"")
            print("  lars ssql test --filter 'quality'")
            print("  lars ssql list --type scalar")
            sys.exit(1)
        else:
        ...
        if args.sql_command == 'query' or args.sql_command == 'q':
            ...
            if len(sys.argv) > 2 and not sys.argv[2].startswith('--'):
                ...
                styled_print(f"{S.WARN}  DEPRECATED: Use 'lars sql query \"SELECT...\"' instead of 'lars sql \"SELECT...\"'")
...
def cmd_render(args):
    ...
    try:
        from lars.terminal_image import render_image_in_terminal
...
def _maybe_render_startup_splash():
    ...
    Shows the semantic SQL server image for 'lars serve sql',
    otherwise shows the default LARS logo.
    """
    if os.environ.get("LARS_NO_SPLASH"):
        return
    ...
    if is_sql_server:
        image_path = SPLASH_DIR / "lars-logo-semantic-sql-server.png"
    else:
        image_path = SPLASH_DIR / "lars-logo-no-bkgrnd.png"
...
def cmd_run(args):
    ...
    if hasattr(args, 'model') and args.model:
        ...
        styled_print(f"{S.CFG} Model override: {args.model}")
    if hasattr(args, 'base_url') and args.base_url:
        overrides['base_url'] = args.base_url
        styled_print(f"{S.CFG} Base URL override: {args.base_url}")
    if hasattr(args, 'api_key') and args.api_key:
        overrides['api_key'] = args.api_key
        styled_print(f"{S.CFG} API key override: ***")
    ...
    from lars.caller_context import build_cli_metadata
    import sys
...
def cmd_test_freeze(args):
    ...
    from lars.testing import SnapshotCapture
    ...
    try:
        ...
        print()
        styled_print(f"{S.OK} Test snapshot created successfully!")
        print()
        print("Next steps:")
        print(f"  • Validate structure: lars test replay {args.name}")
        print(f"  • Validate contracts: lars test replay {args.name} --mode contracts")
        print(f"  • Full validation:    lars test replay {args.name} --mode full")
        print(f"  • Inspect snapshot:   lars test inspect {args.name} --contracts --anchors")
...
def cmd_test_validate(args):
    ...
    try:
        ...
        if result.passed:
            styled_print(f"{S.OK} {result.snapshot_name} PASSED (mode={result.mode})")
            if not args.verbose:
                print(f"  {len(result.checks)} checks passed")
            if result.warnings:
                for warning in result.warnings:
                    styled_print(f"  {S.WARN} {warning}")
...
def cmd_test_run(args):
    ...
    if results["total"] == 0:
        ...
        print()
        print("Create one with: lars test freeze <session_id> --name <name>")
        sys.exit(0)
    ...
    for snapshot_result in results["snapshots"]:
        if snapshot_result["passed"]:
            styled_print(f"  {S.OK} {snapshot_result['name']}")
        else:
...
def cmd_analyze(args):
    ...
    from lars.analyzer import analyze_and_suggest, PromptSuggestionManager
    ...
    try:
        ...
        if args.output:
            ...
            styled_print(f"{S.OK} Suggestions saved to: {filepath}")
        else:
            manager = PromptSuggestionManager()
        ...
        if args.apply:
            ...
            for suggestion in analysis["suggestions"]:
                ...
                if success:
                    styled_print(f"{S.OK} Applied suggestion for cell: {suggestion['cell']}")
                else:
                    print(f"✗ Failed to apply suggestion for cell: {suggestion['cell']}")

            print()
            styled_print(f"{S.OK} All suggestions applied!")
            print()
            ...
            print(f"  lars analyze {args.cascade} --apply")
            print()
...
def cmd_sql(args):
    ...
    try:
        ...
        print("  lars sql \"SELECT * FROM all_data LIMIT 10\"")
        print("  lars sql \"SELECT session_id, SUM(cost) FROM unified_logs GROUP BY session_id\"")
        print("  lars sql \"SELECT * FROM rag WHERE rag_id = 'abc123' LIMIT 5\"")
        sys.exit(1)
...
def cmd_sql_semantic(args):
    ...
    Example usage:
        lars sql semantic "SELECT NORMALIZE('Acme Corp.', 'company')"
        lars sql semantic "SELECT QUALITY('john@gmail.com')"
        lars sql semantic "SELECT PARSE_ADDRESS('123 Main St, Boston MA')"
    """
    import uuid
    from rich.console import Console
    from rich.table import Table
    ...
    try:
        ...
        console.print("  lars sql semantic \"SELECT NORMALIZE('Acme Corp.', 'company')\"")
        console.print("  lars sql semantic \"SELECT QUALITY('john@gmail.com')\"")
        console.print("  lars sql semantic \"SELECT * FROM read_csv('data.csv') WHERE VALID(email, 'email')\"")
        console.print()
...
def cmd_ssql_list(args):
    ...
    for shape in ['scalar', 'aggregate', 'dimension']:
        ...
        for op in sorted(ops, key=lambda x: x['name']):
            test_status = "[green]OK[/green]" if op['has_tests'] else "[dim]--[/dim]"
            if verbose:
...
def cmd_sql_test(args):
    ...
    if mode in ('simple', 'extended', 'all'):
        ...
        if not available:
            ...
            console.print(f"[dim]Start with: lars serve sql --port {args.port}[/dim]")
            if mode != 'all':
                sys.exit(1)
            else:
    ...
    if 'internal' in modes_to_run:
        try:
            ...
            conn = get_session_db(session_id)
            lock = get_session_lock(session_id)
            rewriter_func = rewrite_lars_syntax
    ...
    for cascade_file in sorted(semantic_sql_dir.glob('*.yaml')):
        ...
        for i, test in enumerate(test_cases):
            ...
            if len(modes_to_run) == 1:
                ...
                if r['passed']:
                    if args.verbose:
                        console.print(f"  [green]{S.OK}[/green] {description}")
                else:
                ...
                for run_mode in modes_to_run:
                    ...
                    if r['passed']:
                        status_parts.append(f"[green]{mode_abbrev}:OK[/green]")
                    else:
...
def cmd_db_cleanup_results(args):
    """Drop expired query result tables from lars_results database."""
    from lars.config import get_clickhouse_url
    from lars.db_adapter import get_db_adapter
    ...
    try:
        ...
        # Get list of result tables (r_*) from lars_results database
        tables_query = """
            SELECT name
            FROM system.tables
        ...
        if not result_tables:
            print("No result tables found in lars_results database.")
            print()
            return
        ...
        for table in tables_to_drop:
            try:
                db.execute(f"DROP TABLE IF EXISTS lars_results.{table}")
                dropped += 1
        ...
        if failed > 0:
            ...
            styled_print(f"{S.OK} Dropped {dropped} table(s)")
        print()
...
def cmd_embed_status(args):
    ...
    from lars.db_adapter import get_db_adapter
    from lars.config import get_config
    from lars.embedding_worker import get_embedding_worker, get_embedding_costs
    ...
    print("  export LARS_ENABLE_EMBEDDINGS=true")
    print()
    print("To manually embed recent messages:")
    print("  lars embed run --batch-size 100")
    print()
...
def cmd_embed_run(args):
    ...
    from lars.embedding_worker import EMBEDDING_SESSION_ID, EMBEDDING_CASCADE_ID, EMBEDDING_CELL_NAME
    import uuid
...
def cmd_embed_costs(args):
    ...
    from lars.db_adapter import get_db_adapter
    from lars.embedding_worker import EMBEDDING_CASCADE_ID
    from rich.console import Console
    from rich.table import Table
...
def cmd_hotornot_stats(args):
    ...
    if stats["total_evaluations"] == 0:
        ...
        print("  lars hotornot rate")
        print()
        print("Or quick-rate a session:")
        print("  lars hotornot quick <session_id> good")
        return
...
def cmd_hotornot_list(args):
    ...
    if df.empty:
        ...
        print("  lars examples/takes_flow.json --input '{}'")
        return
...
def cmd_hotornot_rate(args):
    ...
    print()
    print("View stats with: lars hotornot stats")
    print()
...
def cmd_check(args):
    ...
    print()
    print("=" * 70)
    print("LARS Optional Dependencies Check")
    print("=" * 70)
    print()
    ...
    if feature in ['rabbitize', 'all']:
        styled_print(f"{S.WEB} Rabbitize (Visual Browser Automation)")
        print("-" * 70)
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                styled_print(f"  {S.OK} npm: v{npm_version}")
                npm_installed = True
            else:
                print("  ✗ npm: Not found")
                npm_installed = False
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                styled_print(f"  {S.OK} Rabbitize: {version}")
        ...
        try:
            ...
            styled_print(f"  {S.OK} Rabbitize Server: Running")
            server_running = True
        except:
        ...
        if rabbitize_installed and not server_running:
            styled_print(f"  {S.RUN} Start Rabbitize Server:")
            print("     npx rabbitize")
        ...
        if rabbitize_installed and server_running:
            styled_print(f"  {S.DONE} Rabbitize is fully operational!")
            print()
            print("  Try it:")
            print("     lars examples/rabbitize_simple_demo.json --input '{\"url\": \"https://example.com\"}'")
            print()
    ...
    if feature in ['docker', 'all']:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                styled_print(f"  {S.OK} Docker: {docker_version}")
        ...
        try:
            ...
            if 'ubuntu-container' in result.stdout:
                styled_print(f"  {S.OK} ubuntu-container: Exists")
                ...
                if 'ubuntu-container' in result.stdout:
                    styled_print(f"  {S.OK} ubuntu-container: Running")
                    container_running = True
                else:
                    styled_print(f"  {S.WARN}  ubuntu-container: Stopped")
        ...
        if docker_installed and not container_exists:
            styled_print(f"  {S.RUN} Create Ubuntu Container:")
        ...
        if docker_installed and container_exists and not container_running:
            styled_print(f"  {S.RUN} Start Container:")
            print("     docker start ubuntu-container")
            print()

        if docker_installed and container_running:
            styled_print(f"  {S.DONE} Docker environment is ready!")
            print()
            print("  Try it:")
            print("     lars examples/simple_flow.json --input '{\"data\": \"test\"}'")
            print()
    ...
    for name, status in checks:
        if status:
            styled_print(f"  {S.DONE} {name}: Ready")
        else:
            styled_print(f"  {S.ERR} {name}: Not Ready")
...
def cmd_harbor_list(args):
    ...
    try:
        from lars.harbor import list_user_spaces
        from lars.config import get_config
        ...
        for space in spaces:
            status_icon = {
                ...
                "BUILDING": "[yellow]BUILD[/yellow]",
                "PAUSED": "[yellow]PAUSED[/yellow]",
            }.get(space.status, "[dim]?[/dim]")
...
def cmd_harbor_introspect(args):
    ...
    try:
        from lars.harbor import get_space_endpoints
        from lars.config import get_config
...
def cmd_harbor_export(args):
    ...
    try:
        from lars.harbor import export_tool_definition
...
def cmd_harbor_manifest(args):
    ...
    try:
        from lars.harbor import get_harbor_manifest, format_harbor_manifest
        from lars.config import get_config
...
def cmd_harbor_wake(args):
    ...
    try:
        from lars.harbor import wake_space
...
def cmd_harbor_pause(args):
    ...
    try:
        from lars.harbor import pause_space
...
def cmd_harbor_status(args):
    ...
    try:
        from lars.harbor import get_spaces_summary, HARDWARE_PRICING
        from lars.config import get_config
        ...
        for space in sorted_spaces:
            ...
            private_marker = " [PRIVATE]" if space.private else ""
        ...
        print("[PRIVATE] = Private space")
        print()
        print("Commands:")
        print("  lars harbor wake <space>   - Wake a sleeping space")
        print("  lars harbor pause <space>  - Pause a running space (stops billing)")
        print("  lars harbor introspect <space> - View API endpoints")
...
def cmd_triggers_list(args):
    ...
    from lars.triggers import list_triggers
    ...
    try:
        ...
        for trigger in triggers:
            enabled = "[green]ON[/green]" if trigger.get("enabled", True) else "[red]OFF[/red]"
            styled_print(f"  {enabled} {trigger['name']} ({trigger['type']})")
        ...
        print(f"  lars triggers export {args.cascade} --format cron")
        print(f"  lars triggers export {args.cascade} --format kubernetes")
        print()
...
def cmd_triggers_export(args):
    ...
    from lars.triggers import export_cron, export_systemd, export_kubernetes, export_airflow
    import os
...
def cmd_triggers_check(args):
    ...
    from lars.triggers import check_sensor, get_trigger
    from lars.cascade import SensorTrigger
    ...
    try:
        ...
        if is_ready:
            styled_print(f"{S.OK} Condition MET - ready to trigger")
            print()
...
def cmd_signals_list(args):
    ...
    try:
        ...
        print("  lars signals fire <signal_name> --payload '{\"key\": \"value\"}'")
...
def cmd_signals_fire(args):
    ...
    try:
        ...
        if count > 0:
            styled_print(f"{S.OK} Fired signal '{args.signal_name}' - woke up {count} waiting cascade(s)")
            if payload:
...
def cmd_signals_status(args):
    ...
    from lars.signals import get_signal_manager
...
def cmd_signals_cancel(args):
    ...
    try:
        ...
        styled_print(f"{S.OK} Cancelled signal '{args.signal_id}'")
...
def cmd_sessions_list(args):
    ...
    from lars.session_state import get_session_state_manager, SessionStatus
    ...
    try:
        ...
        if running_count > 0:
            print(f"\n{running_count} running - to cancel: lars sessions cancel <session_id>")
        if blocked_count > 0:
            print(f"{blocked_count} blocked - see details: lars sessions show <session_id>")
...
def cmd_sessions_show(args):
    ...
    from lars.session_state import get_session_state_manager
...
def cmd_sessions_cancel(args):
    ...
    try:
        ...
        styled_print(f"{S.OK} Cancellation requested for session '{args.session_id}'")
        print()
...
def cmd_sessions_cleanup(args):
    ...
    try:
        ...
        if args.dry_run:
            ...
            print("  lars sessions cleanup")
        else:
            count = manager.cleanup_zombies(grace_period_seconds=args.grace)
            styled_print(f"{S.OK} Marked {count} session(s) as orphaned")
...
def cmd_models_list(args):
    ...
    from lars.models_mgmt import list_models
...
def cmd_models_verify(args):
    ...
    from lars.models_mgmt import verify_models
...
def cmd_models_stats(args):
    ...
    from lars.models_mgmt import show_stats
...
def cmd_models_local_status(args):
    ...
    try:
        from lars.local_models import is_available, get_device_info, get_model_registry
    except ImportError:
        console.print("[red]Local models module not available.[/red]")
        console.print("Install with: pip install lars\\[local-models]")
        sys.exit(1)
...
def cmd_models_local_list(args):
    ...
    try:
        from lars.local_models import is_available, get_model_registry
    except ImportError:
        console.print("[red]Local models module not available.[/red]")
        console.print("Install with: pip install lars\\[local-models]")
        sys.exit(1)
...
def cmd_models_local_load(args):
    ...
    try:
        ...
        console.print(f"[green]{S.OK} Model loaded successfully[/green]")
...
def cmd_models_local_unload(args):
    ...
    if registry.unload(args.model_id):
        console.print(f"[green]{S.OK} Model '{args.model_id}' unloaded[/green]")
    else:
...
def cmd_models_local_clear(args):
    ...
    console.print(f"[green]{S.OK} Cleared {count} model(s) from cache[/green]")
...
def cmd_models_local_export(args):
    ...
    if args.output:
        with open(args.output, 'w') as f:
            ...
            f.write("# Install dependencies: pip install lars[local-models]\n\n")
            f.write(yaml_str)
        console.print(f"[green]{S.OK} Tool definition saved to {args.output}[/green]")
    else:
...
def cmd_harbor_refresh(args):
    ...
    from lars.harbor_mgmt import refresh_spaces
...
def cmd_harbor_list_cached(args):
    ...
    from lars.harbor_mgmt import list_spaces
...
def cmd_tools_list(args):
    ...
    from lars.tools_mgmt import list_tools
...
def cmd_tools_usage(args):
    ...
    from lars.tools_mgmt import show_usage_stats
...
def cmd_tools_stats(args):
    ...
    from lars.tools_mgmt import show_tool_stats
...
def cmd_tools_search(args):
    ...
    from lars.tools_mgmt import find_tool_by_description
...
def cmd_tools_find(args):
    ...
    from lars.tools_mgmt import semantic_find_tools
...
def cmd_cache_clear(args):
    ...
    if not args.function and not args.older_than and not args.all:
        ...
        console.print("   lars cache clear --function semantic_summarize")
        console.print("   lars cache clear --older-than 7")
        console.print("   lars cache clear --all")
        return
...
def cmd_serve_watcher(args):
    """Start the LARS WATCH daemon for reactive SQL subscriptions."""
    from lars.watcher import run_daemon
    from lars.db_adapter import ensure_housekeeping

    # Initialize database (create tables if needed)
    styled_print(f"{S.CFG} Initializing database...")
    ensure_housekeeping()

    print()
    print("=" * 60)
    print("  LARS WATCH DAEMON")
    print("=" * 60)
    print()
...
def cmd_serve_browser(args):
    """Start the LARS browser automation server."""
    try:
        ...
        print("  pip install lars[browser]")
...
def cmd_browser_commands(args):
    ...
    try:
        from lars.browser.commands import get_available_commands, get_command_help
    except ImportError as e:
        print("✗ Browser module not available.")
        print(f"Error: {e}")
        sys.exit(1)
...
def cmd_browser_batch(args):
    ...
    try:
        ...
        print("✗ Browser module not available.")
        print(f"  Install with: pip install lars[browser]")
    ...
    styled_print(f"{S.WEB} Starting batch browser session...", file=sys.stderr)
    ...
    async def run_batch():
        ...
        try:
            ...
            styled_print(f"{S.OK} Browser initialized", file=sys.stderr)
            ...
            for i, cmd in enumerate(commands):
                ...
                try:
                    ...
                    if not cmd_result.get("success", True):
                        styled_print(f"    {S.WARN} Warning: {cmd_result.get('error', 'unknown error')}", file=sys.stderr)
                except Exception as e:
                    print(f"    ✗ Error: {e}", file=sys.stderr)

            print(file=sys.stderr)
            styled_print(f"{S.OK} Completed {len(commands)} commands", file=sys.stderr)
            ...
            styled_print(f"{S.OK} Session closed", file=sys.stderr)
...
def cmd_init(args):
    ...
    print(f"Initializing LARS workspace at {workspace}")
    print()
    ...
    if env_example.exists() and not env_file.exists():
        ...
        # Update LARS_ROOT to absolute path
        content = env_file.read_text()
        content = content.replace('LARS_ROOT=.', f'LARS_ROOT={workspace}')
