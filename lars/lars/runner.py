...
_current_hooks: ContextVar[Optional['LARSHooks']] = ContextVar('current_hooks', default=None)

def get_current_hooks() -> Optional['LARSHooks']:
    """Get the current hooks from context."""
    return _current_hooks.get()

def set_current_hooks(hooks: Optional['LARSHooks']):
...
class LARSHooks:
    """Base class for LARS lifecycle hooks"""
...
class LARSRunner:
    def __init__(self, config_path: str | dict, session_id: str = "default", overrides: dict | None = None,
                 depth: int = 0, parent_trace: TraceNode | None = None, hooks: LARSHooks | None = None,
    ...
    def _execute_cells_parallel(self, input_data: dict) -> dict:
        ...
        indent = "  " * self.depth
        console.print(f"{indent}[bold magenta]{S.PARALLEL} Parallel execution mode enabled[/bold magenta]")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            while len(completed) < len(self.config.cells):
                ...
                if self._check_cancellation():
                    console.print(f"{indent}[bold yellow]{S.WARN} Cascade cancelled during parallel execution[/bold yellow]")
                    break
                ...
                for cell_name in runnable:
                    console.print(f"{indent}  [cyan][EXEC] Starting: {cell_name}[/cyan]")
                ...
                for future in done:
                    ...
                    if result["success"]:
                        console.print(f"{indent}  [green][OK] Completed: {cell_name}[/green]")
                    else:
                        console.print(f"{indent}  [red]✗ Failed: {cell_name}[/red]")

        console.print(f"{indent}[bold magenta]{S.PARALLEL} Parallel execution complete: {len(completed)}/{len(self.config.cells)} cells[/bold magenta]")
    ...
    def _log_context_selection(
        ...
        selection_emoji = "{S.COMPRESS}" if stats.selection_type == "standard" else "{S.RETRY}" if stats.selection_type == "loop_retry" else "{S.NOTES}"
    ...
    def _inject_audible_feedback(self, feedback: dict, cell: 'CellConfig', trace: 'TraceNode'):
        ...
        console.print(f"  [bold green][OK] Audible feedback injected ({mode} mode)[/bold green]")
    ...
    def _build_auto_context(self, cell: CellConfig, input_data: dict, trace: 'TraceNode') -> List[Dict]:
        ...
        console.print(f"  [bold magenta]{S.THINK} Auto-context mode: intelligent selection...[/bold magenta]")

        # Log auto-context start as structural message
        auto_ctx_trace = trace.create_child("auto_context", "inter_cell_selection")
        self.echo.add_history({
            "role": "structure",
            "content": f"{S.THINK} Auto-Context: Inter-Cell Selection",
        ...
        self.echo.add_history({
            "role": "structure",
            "content": (
                f"{S.DONE} Auto-Context Selection Complete\n"
    ...
    def _handle_human_input_checkpoint(
        ...
        indent = "  " * self.depth
        console.print(f"{indent}[bold yellow]{S.PAUSE}  Human checkpoint: {cell.name}[/bold yellow]")
        ...
        if config.condition:
            try:
                ...
                console.print(f"{indent}  [yellow]{S.WARN}  Condition evaluation error: {e}[/yellow]")
        ...
        if response is None:
            ...
            console.print(f"{indent}  [yellow]{S.WARN}  No human response received[/yellow]")
            self.echo.add_history(
        ...
        # Response received!
        console.print(f"{indent}  [green][OK] Human response received[/green]")
        self.echo.add_history(
    ...
    def _check_for_decision_point(
        ...
        try:
            ...
            if not decision.get("question") or not decision.get("options"):
                console.print(f"[yellow]{S.WARN} Invalid decision block: missing question or options[/yellow]")
                return None
            ...
            console.print(f"[yellow]{S.WARN} Invalid JSON in decision block: {e}[/yellow]")
            return None
    ...
    def _handle_decision_point(
        ...
        if response is None:
            console.print(f"{indent}  [yellow]{S.WARN} Decision timed out or cancelled[/yellow]")
            self.echo.add_history(
        ...
        console.print(f"{indent}  [green][OK] Decision received: {selected_id}[/green]")
...
"""
    ...
    def _parse_prompt_tool_calls(self, content: str) -> tuple[List[Dict], str]:
        ...
        for match in re.finditer(html_fence_pattern, content, re.DOTALL | re.IGNORECASE):
            ...
            console.print(
                f"[yellow]{S.WARN}  AUTO-CORRECTED: Model output HTML in code fence instead of "
        ...
        if not tool_calls:
            ...
            if has_interactive_html:
                ...
                if len(html_content) > 100 and '<' in html_content and '>' in html_content:
                    ...
                    console.print(
                        f"[yellow]{S.WARN}  AUTO-CORRECTED: Model output raw HTML instead of "
                        f"using request_decision tool. Injecting into request_decision call.[/yellow]"
                    )
    ...
    def _run_with_cascade_takes(self, input_data: dict | None = None) -> dict:
        ...
        if isinstance(self.config.takes.factor, str):
            ...
            try:
                ...
                console.print(f"{indent}[red]{S.WARN} Warning: Could not parse cascade takes factor '{rendered_factor}' as integer, defaulting to 1[/red]")
                factor = 1
        else:
        ...
        self.echo.add_history({
            "role": "cascade_takes",
            "content": f"[TAKE] Running {factor} cascade takes",
        ...
        def run_single_cascade_take(i: int) -> dict:
            ...
            try:
                ...
                console.print(f"{indent}    [green][OK] Cascade Take {i+1} complete[/green]")
        ...
        console.print(f"{indent}[bold yellow]{S.EVAL}  Evaluating {len(take_results)} cascade executions...[/bold yellow]")
        ...
        console.print(f"{indent}[bold green]{S.WIN} Winner: Cascade Take {winner_index + 1}[/bold green]")
        ...
        self.echo.add_history({
            ...
            "content": f"{S.WIN} Winner: Cascade #{winner_index + 1}",
...
"""
            ...
            if reforge_config.mutate:
                ...
                console.print(f"{indent}  [yellow]{S.MUT} Mutation applied: {mutation_prompt[:60]}...[/yellow]")
            ...
            def run_single_refinement(i: int) -> dict:
                ...
                try:
                    ...
                    console.print(f"{indent}      [green][OK] Cascade Refinement {i+1} complete[/green]")
            ...
            console.print(f"{indent}    [bold yellow]{S.EVAL}  Evaluating cascade refinements...[/bold yellow]")
            ...
            console.print(f"{indent}    [bold green]{S.WIN} Best Cascade Refinement: #{winner_index + 1}[/bold green]")
            ...
            if reforge_config.threshold:
                console.print(f"{indent}    [cyan]{S.WARD}  Checking cascade reforge threshold...[/cyan]")
                ...
                if threshold_result['valid']:
                    console.print(f"{indent}    [bold green]{S.DONE} Cascade threshold met! Stopping reforge early at step {step}[/bold green]")
    ...
    def _run_quartermaster(self, cell: CellConfig, input_data: dict, trace: TraceNode, cell_model: str | None = None) -> list[str]:
        ...
        if len(manifest) > cell.manifest_limit:
            try:
                ...
                console.print(f"{indent}  [dim cyan]{S.SEARCH} Semantic pre-filtering ({len(manifest)} → {cell.manifest_limit} tools)...[/dim cyan]")
                ...
                if relevant_tools:
                    ...
                    console.print(f"{indent}  [green][OK][/green] Pre-filtered to {len(filtered_manifest)} most relevant tools")
                else:
                    console.print(f"{indent}  [yellow]{S.WARN}[/yellow] No embeddings found, using full manifest")

            except Exception as e:
                console.print(f"{indent}  [yellow]{S.WARN}[/yellow] Semantic filtering failed: {e}")
...
"""
    ...
    def _rewrite_prompt_with_llm(self, cell: CellConfig, input_data: dict, mutation_template: str, parent_trace: TraceNode, mutation_mode: str = "rewrite", species_hash: str | None = None) -> str:
        ...
        if mutation_mode == "rewrite":
            ...
            limit = int(os.environ.get("LARS_WINNER_HISTORY_LIMIT", "5"))
            ...
            if winners:
                console.print(f"{indent}    [dim cyan][INFO] Learning from {len(winners)} previous winning rewrites[/dim cyan]")
...
"""
                ...
                console.print(f"{indent}    [dim]{S.SEARCH} No previous winners - exploratory rewrite[/dim]")

        elif mutation_mode == "rewrite_free":
            console.print(f"{indent}    [dim]{S.SEARCH} Pure exploration mode (rewrite_free)[/dim]")
...
## Rewritten Prompt:"""
        ...
        # This can be overridden via LARS_REWRITE_MODEL env var
        ...
        if not rewritten_prompt:
            console.print(f"{indent}    [yellow]{S.WARN} Rewrite failed, using original prompt[/yellow]")
...
'''
            ...
            # Log result
            if is_valid:
                console.print(f"{indent}    [bold green][OK] PASSED:[/bold green] {reason}")
            else:
    ...
    def _run_ward(self, ward_config, content: str, trace: TraceNode, ward_type: str = "post") -> dict:
        ...
        if isinstance(validator_spec, PolyglotValidatorConfig):
            ...
            icon = mode_icons.get(mode, "{S.WARD}")
    ...
    def _run_loop_until_validator(
        ...
        if is_per_turn and is_valid:
            console.print(f"{indent}  [bold green][OK] Early exit: {validator_name} passed on turn {turn + 1}[/bold green]")
        elif not is_per_turn:
            if is_valid:
                console.print(f"{indent}  [bold green][OK] Validator passed: {reason[:100]}[/bold green]")
            else:
    ...
    def _aggregate_with_llm(self, outputs: List[Dict], instructions: str, model: str, trace: TraceNode) -> str:
        ...
        for i, o in enumerate(outputs):
            ...
            outputs_text += f"\n{o['output']}"
            if o.get('images'):
                outputs_text += f"\n([SNAP] {len(o['images'])} image(s) attached below)"
...
Please combine/synthesize these outputs according to the instructions above. Produce a single coherent output that captures the best aspects of each."""
        ...
        if any_images:
            ...
            console.print(f"{indent}  [cyan][SNAP] Multi-modal aggregation: {total_images} total images[/cyan]")
    ...
    def _execute_cell_with_takes(self, cell: CellConfig, input_data: dict, trace: TraceNode, initial_injection: dict | None = None) -> Any:
        ...
        if isinstance(cell.takes.factor, str):
            ...
            try:
                ...
                console.print(f"{indent}[red]{S.WARN} Warning: Could not parse takes factor '{rendered_factor}' as integer, defaulting to 1[/red]")
                resolved_factor = 1
        else:
        ...
        if cell.takes.models and len(set(assigned_models)) > 1:
            ...
            if filter_result["estimated_tokens"] > 0:
                ...
                for model in unique_models_checked:
                    ...
                    if filter_detail:
                        ...
                        if limit:
                            console.print(f"{indent}  [dim green][OK] {model}: {limit:,} token limit[/dim green]")
                        else:
                            console.print(f"{indent}  [dim green][OK] {model}: sufficient context[/dim green]")
            ...
            if filtered_count > 0:
                console.print(
                    f"{indent}  [yellow]{S.FAST} Filtered {filtered_count} model(s) with insufficient context[/yellow]"
                )
        ...
        def run_single_take(i: int) -> dict:
            ...
            console.print(f"{indent}  [cyan]{S.CASCADE} Take {i+1}/{factor}[/cyan]" +
                         (f" [yellow]{S.MUT} {mutation_info['type']}[/yellow]" if mutation_info['type'] else " [dim](baseline)[/dim]") +
            ...
            try:
                ...
                console.print(f"{indent}    [green][OK] Take {i+1} complete[/green]")
        ...
        if cell.takes.validator:
            ...
            console.print(f"{indent}[bold cyan]{S.SEARCH} Pre-evaluation validation with '{validator_display_name}'...[/bold cyan]")
            ...
            for sr in take_results:
                ...
                if validation["valid"]:
                    console.print(f"{indent}  [green][OK] Take {sr['index']+1}: VALID[/green] - {validation['reason'][:60]}...")
                else:
            ...
            if not valid_take_results:
                console.print(f"{indent}[bold red]{S.WARN}  All {len(take_results)} takes failed validation![/bold red]")
                ...
                console.print(f"{indent}[bold green][OK] {len(valid_take_results)}/{len(take_results)} takes passed validation[/bold green]")
        ...
        if cell.takes.mode == "aggregate":
            ...
            console.print(f"{indent}[bold green][OK] Aggregated {len(all_outputs)} outputs[/bold green]")
        ...
        if use_human_eval or use_hybrid_eval:
            ...
            console.print(f"{indent}  [bold]Open the LARS UI to select a winner[/bold]")
            ...
            if response is None:
                ...
                console.print(f"{indent}[yellow]{S.WARN} Human evaluation timed out or cancelled[/yellow]")
                ...
                if response.get("reject_all"):
                    ...
                    console.print(f"{indent}[yellow]{S.WARN} Human rejected all takes[/yellow]")
                ...
                console.print(f"{indent}[bold green][OK] Human selected: Take {winner_take['index'] + 1}[/bold green]")
                if reasoning:
        ...
        if not (use_human_eval or use_hybrid_eval or winner_already_set):
            console.print(f"{indent}[bold yellow]{S.EVAL}  Evaluating {len(valid_take_results)} takes...[/bold yellow]")

            # Cell 3: Pareto Frontier Analysis
            if use_pareto:
                assert cell.takes.pareto_frontier is not None  # Guarded by use_pareto check
                console.print(f"{indent}  [bold cyan]{S.CHART} Computing Pareto Frontier...[/bold cyan]")
...
"""
            ...
            # Cell 2: Cost-Aware Evaluation
                ...
                if any_images:
                    ...
                    console.print(f"{indent}  [cyan][SNAP] Multi-modal evaluation: {sum(len(s.get('images', [])) for s in valid_take_results)} total images[/cyan]")
            ...
            # Cell 1: Standard quality-only evaluation
                ...
                if any_images:
                    ...
                    for i, take in enumerate(valid_take_results):
                        ...
                        if num_images > 0:
                            header += f"\n\n[SNAP] Visual Output ({num_images} image{'s' if num_images > 1 else ''} follow):"
        ...
        if winner.get("is_aggregated"):
            ...
            console.print(f"{indent}[bold green]{S.WIN} Winner: Take {winner['index'] + 1}[/bold green]")
        ...
        if cell.takes.reforge:
            ...
            if is_aggregated:
                console.print(f"{indent}[yellow]{S.WARN}  Reforge skipped: Not compatible with aggregate mode[/yellow]")
            else:
                # Track which take won so reforge messages can reference it
            ...
            for i, (img_data, desc) in enumerate(images_to_save):
                ...
                if not os.path.exists(save_path):
                    try:
                        ...
                        console.print(f"{indent}    [dim]{S.SAVE} Saved image: {save_path}[/dim]")
                        self._maybe_render_image_to_console(save_path, indent)
                    except Exception as e:
                        console.print(f"{indent}    [dim yellow]{S.WARN}  Failed to save image: {e}[/dim yellow]")
    ...
    def _maybe_render_image_to_console(self, image_path: str, indent: str = ""):
        ...
        if not os.environ.get("LARS_SHOW_CLI_IMAGES", "true").lower() in ("true", "1"):
            return
        ...
        try:
            ...
            console.print(f"{indent}    [dim cyan]{S.VIEW}  Rendering image:[/dim cyan]")
            ...
            for i, (vid_data, desc) in enumerate(videos_to_save):
                ...
                if not os.path.exists(save_path):
                    try:
                        ...
                        console.print(f"{indent}    [dim]{S.VIDEO} Saved video: {save_path}[/dim]")
                    except Exception as e:
                        console.print(f"{indent}    [dim yellow]{S.WARN}  Failed to save video: {e}[/dim yellow]")
...
"""
            ...
            def run_single_refinement(i: int) -> dict:
                ...
                try:
                    ...
                    console.print(f"{indent}      [green][OK] Refinement {i+1} complete[/green]")
            ...
            # Evaluate refinements
            console.print(f"{indent}    [bold yellow]{S.EVAL}  Evaluating refinements...[/bold yellow]")
            ...
            if any_images:
                ...
                console.print(f"{indent}    [cyan][SNAP] Multi-modal reforge evaluation: {sum(len(r.get('images', [])) for r in reforge_results)} total images[/cyan]")
            else:
                # Text-only evaluation (original behavior)
                eval_context_messages = []
                for i, refinement in enumerate(reforge_results):
            ...
            console.print(f"{indent}    [bold green]{S.WIN} Best Refinement: #{winner_index + 1}[/bold green]")
            ...
            self.echo.add_history({
                "role": "reforge_winner",
                "content": f"{S.WIN} Step {step} Winner: #{winner_index + 1}",
            ...
            # Check threshold ward if configured
            if reforge_config.threshold:
                console.print(f"{indent}    [cyan]{S.WARD}  Checking reforge threshold...[/cyan]")
                ...
                if threshold_result['valid']:
                    console.print(f"{indent}    [bold green]{S.DONE} Threshold met! Stopping reforge early at step {step}[/bold green]")
    ...
    def _setup_browser_session(self, cell: CellConfig, input_data: dict, trace: TraceNode) -> Optional[BrowserSession]:
        ...
        indent = "  " * self.depth
        console.print(f"{indent}[bold cyan]{S.WEB} Starting browser session for cell '{cell.name}'[/bold cyan]")
            ...
            try:
                browser_session = loop.run_until_complete(
                    create_browser_session(
                        ...
                        cascade_id=self.config.cascade_id,
                        cell_name=cell.name,
                        lars_session_id=self.session_id
                    )
                )
                ...
                console.print(f"{indent}[green][OK] Browser ready at {url}[/green]")
    ...
    def _teardown_browser_session(self, browser_session: BrowserSession, cell: CellConfig) -> None:
        ...
        indent = "  " * self.depth
        console.print(f"{indent}[dim]{S.WEB} Closing browser session...[/dim]")
        ...
        try:
            ...
            console.print(f"{indent}[green][OK] Browser session closed[/green]")

        except Exception as e:
            console.print(f"{indent}[yellow]{S.WARN} Error closing browser: {e}[/yellow]")
    ...
    def _execute_image_generation_cell(self, cell: CellConfig, input_data: dict, trace: TraceNode) -> Any:
        ...
        console.print(f"{indent}[bold magenta]{S.ART} Image Generation: {cell.name}[/bold magenta]")
        ...
        if cell.context and cell.context.from_:
            ...
            for source in cell.context.from_:
                ...
                if source_name:
                    ...
                    if source_images:
                        console.print(f"{indent}  [cyan][SNAP] Loaded {len(source_images)} image(s) from cell '{source_name}'[/cyan]")
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if is_transient and attempt < max_attempts - 1:
                    ...
                    console.print(f"{indent}  [yellow]{S.WARN} Image generation failed (attempt {attempt + 1}/{max_attempts}), retrying in {wait_time}s...[/yellow]")
        ...
        try:
                        ...
                        if url.startswith("data:"):
                            ...
                            try:
                                ...
                                if fingerprint in seen_fingerprints:
                                    console.print(f"{indent}    [dim]{S.SKIP}  Skipping duplicate image[/dim]")
                                    continue
                                seen_fingerprints.add(fingerprint)
            ...
            # Console output
            console.print(f"{indent}  [green][OK] Generated {len(saved_paths)} image(s)[/green]")
...
Return ONLY the corrected Python code. No explanations, no markdown code blocks, just the raw code."""
        ...
        for attempt in range(config.max_attempts):
            ...
            try:
                ...
                # Success!
                console.print(f"{indent}    [green][OK] Auto-fix succeeded on attempt {attempt + 1}[/green]")
    ...
    def _execute_sql_mapping_cell(self, cell: CellConfig, input_data: dict, trace: TraceNode) -> Any:
        ...
        console.print(f"{indent}[bold magenta]{S.MAP}  SQL Mapping: {config.table} → {config.max_parallel} parallel[/bold magenta]")
        ...
        console.print(f"{indent}  [bold green][OK] Processed {successful_count}/{total_rows} rows[/bold green]")
        ...
        if config.result_table and len(results) > 0:
            try:
                ...
                console.print(f"{indent}  [yellow]{S.WARN} Warning: Could not create result table: {e}[/yellow]")
        ...
        try:
            ...
            if cell.on_error:
                ...
                if auto_fix_config and auto_fix_config.enabled:
                    # Attempt auto-fix
                    console.print(f"{indent}  [yellow][CFG] Attempting auto-fix...[/yellow]")
                    try:
    ...
    def _execute_cell_internal(self, cell: CellConfig, input_data: dict, trace: TraceNode, initial_injection: dict | None = None, mutation: str | None = None, mutation_mode: str | None = None, pre_built_context: list | None = None) -> Any:
        ...
        if cell.use_training:
            try:
                ...
                console.print(f"{indent}[yellow]{S.WARN}  Training example injection failed: {e}[/yellow]")
        ...
        if cell.wards and cell.wards.pre:
            console.print(f"{indent}[bold cyan]{S.WARD}  Running Pre-Wards (Input Validation)...[/bold cyan]")
            ...
            for ward_idx, ward_config in enumerate(cell.wards.pre):
                ...
                if not ward_result["valid"]:
                    ...
                    if ward_result["mode"] == "blocking":
                        ...
                        console.print(f"{indent}  [yellow]{S.WARN}  Advisory warning (continuing)...[/yellow]")
        ...
        if is_manifest:
            console.print(f"{indent}  [bold cyan]{S.MAP}  Quartermaster charting skills...[/bold cyan]")
            skill_list = self._run_quartermaster(cell, input_data, trace, cell_model)
            console.print(f"{indent}  [bold cyan]{S.CLIP} Manifest: {', '.join(skill_list)}[/bold cyan]")
        ...
        if initial_injection and initial_injection.get("action") == HookAction.INJECT:
            ...
            console.print(f"{indent}[bold red]{S.FAST} Injection Triggered:[/bold red] {inject_content}")
        ...
        if cell.async_cascades:
            for sub in cell.async_cascades:
                if sub.trigger == "on_start":
                    ...
                    console.print(f"{indent}  {S.HOT} [bold orange1]Spawning Side-Effect: {sub.ref}[/bold orange1]")

                    # Resolve path for async_cascades
                    ref_path = sub.ref
                    if not os.path.isabs(ref_path):
                        if ref_path.startswith("lars/"): # If it's already project-root-relative
        ...
        for attempt in range(max_attempts):
            ...
            if attempt > 0:
                console.print(f"{indent}[bold yellow]{S.RETRY} Validation Retry Attempt {attempt + 1}/{max_attempts}[/bold yellow]")
                ...
                if (
                    ...
                    console.print(f"{indent}  [bold cyan]{S.MAP}  Quartermaster re-charting skills for attempt {attempt + 1}...[/bold cyan]")
                    ...
                    if new_tools or removed_tools:
                        console.print(f"{indent}  [bold cyan]{S.CLIP} Manifest update: +{len(new_tools)} -{len(removed_tools)} tools[/bold cyan]")
            ...
            for i in range(max_turns):
                ...
                if hook_result.get("action") == HookAction.INJECT:
                    ...
                    console.print(f"{indent}[bold red]{S.FAST} Turn Injection:[/bold red] {turn_injection}")
                ...
                if (
                    ...
                    console.print(f"{indent}  [bold cyan]{S.MAP}  Quartermaster re-charting skills for turn {i+1}...[/bold cyan]")
                ...
                try:
                    ...
                    for infra_attempt in range(infrastructure_max_retries):
                        if infra_attempt > 0:
                            console.print(f"{indent}  [bold yellow]{S.RETRY} Infrastructure Retry {infra_attempt + 1}/{infrastructure_max_retries}[/bold yellow]")
                        ...
                        try:
                            ...
                            keep_images = int(os.getenv('LARS_KEEP_RECENT_IMAGES', '0'))
                            keep_turns = int(os.getenv('LARS_KEEP_RECENT_TURNS', '0'))
                            ...
                            if self.token_manager:
                                ...
                                if budget_status["warning"]:
                                    ...
                                    console.print(f"{indent}  [yellow]{S.WARN}  Token budget: {percentage:.1f}% used ({budget_status['current']}/{budget_status['limit']} tokens)[/yellow]")

                                if budget_status["over_budget"]:
                                    console.print(f"{indent}  [red]{S.BOOM} Token budget exceeded, enforcing with strategy: {self.config.token_budget.strategy}[/red]")
                            ...
                            if context_stats.selection_type != "disabled":
                                ...
                                if context_stats.tokens_saved > 0:
                                    console.print(f"{indent}  [dim cyan]{S.COMPRESS}  Auto-context: {context_stats.context_size}/{context_stats.full_history_size} msgs, ~{context_stats.tokens_saved} tokens saved[/dim cyan]")
                            ...
                            if (not content or content.strip() == "") and not tool_calls and not images:
                                ...
                                console.print(f"{indent}  [bold red]{S.WARN}  Infrastructure Error: {error_msg}[/bold red]")
                            ...
                            if images and (not content or content.strip() == ""):
                                ...
                                if not ModelRegistry.is_image_output_model(cell_model):
                                    console.print(f"{indent}  [cyan][SNAP] Runtime detected image model: {cell_model}[/cyan]")
                            ...
                            if is_infrastructure_error:
                                ...
                                console.print(f"{indent}  [bold yellow]{S.WARN}  Infrastructure Error: {infra_error}[/bold yellow]")
                    ...
                    if not use_native and not tool_calls:
                        ...
                        if parse_error:
                            ...
                            console.print(f"{indent}  [bold red]{S.WARN}  JSON Parse Error:[/bold red] {parse_error}")
                    ...
                    if tool_calls and not json_parse_error:
                        ...
                        for tc in tool_calls:
                            ...
                            if isinstance(parsed_result, dict) and "images" in parsed_result:
                                ...
                                if valid_images > 0:
                                    ...
                                    console.print(f"{indent}    [bold magenta][SNAP] Injecting {valid_images} images into next turn[/bold magenta]")
                                ...
                                for i, audio_path in enumerate(audio_files):
                                    if os.path.exists(audio_path):
                                        ...
                                        try:
                                            ...
                                            console.print(f"{indent}    [dim yellow]{S.WARN}  Failed to save audio: {e}[/dim yellow]")
                                    else:
                                        console.print(f"{indent}    [dim yellow]{S.WARN}  Audio file not found: {audio_path}[/dim yellow]")
                        ...
                        # Note: Only cull if LARS_KEEP_RECENT_IMAGES is explicitly set > 0
                        if keep_images > 0:
                            self.context_messages = cull_old_base64_images(self.context_messages, keep_recent=keep_images)
                    ...
                    if self._check_audible_signal(cell):
                        ...
                        if feedback:
                            ...
                            if mode == "retry":
                                console.print(f"{indent}  [bold yellow]{S.RETRY} Retry mode - redoing turn {i + 1}[/bold yellow]")
                    ...
                    if cell.rules.loop_until and i < max_turns - 1:
                        ...
                        if per_turn_result.get("valid"):
                            ...
                            console.print(f"{indent}  [bold cyan]{S.RUN} Early exit: Task complete after turn {i + 1}/{max_turns}[/bold cyan]")
            ...
            if cell.output_schema:
                console.print(f"{indent}[bold cyan]{S.CLIP} Validating Output Schema...[/bold cyan]")
                ...
                try:
                    ...
                    console.print(f"{indent}  [bold green][OK] Schema Validation Passed[/bold green]")
                    ...
                    self.echo.add_history({
                        ...
                        "content": "[OK] Schema validation passed",
                    ...
                    if attempt + 1 >= max_attempts:
                        console.print(f"{indent}[bold red]{S.WARN}  Max schema validation attempts reached ({max_attempts})[/bold red]")
                    else:
            ...
            if cell.rules.loop_until and not validation_passed:
                validator_spec = cell.rules.loop_until
                validator_display_name = _format_validator_name(validator_spec)
                console.print(f"{indent}[bold cyan]{S.WARD}  Running Validator: {validator_display_name}[/bold cyan]")
                ...
                self.echo.add_history({
                    "role": "validation",
                    "content": f"{S.WARD} Running validator: {validator_display_name}",
                ...
                self.echo.add_history({
                    "role": "validation",
                    "content": f"{'[OK]' if is_valid else '✗'} {validator_name}: {reason[:200]}",
                ...
                if is_valid:
                    console.print(f"{indent}  [bold green][OK] Validation Passed:[/bold green] {reason[:150]}...")
                    validation_passed = True
                    ...
                    # If this was the last attempt, we're done
                    if attempt + 1 >= max_attempts:
                        console.print(f"{indent}[bold red]{S.WARN}  Max validation attempts reached ({max_attempts})[/bold red]")

            # ========== POST-WARDS: Validate outputs after cell completes ==========
            post_ward_retry_needed = False
            if cell.wards and cell.wards.post:
                console.print(f"{indent}[bold cyan]{S.WARD}  Running Post-Wards (Output Validation)...[/bold cyan]")
                ...
                for ward_idx, ward_config in enumerate(cell.wards.post):
                    ...
                    if not ward_result["valid"]:
                        ...
                        if ward_result["mode"] == "blocking":
                            ...
                            if attempt + 1 < max_attempts:
                                console.print(f"{indent}  [yellow]{S.RETRY} Post-ward will trigger retry...[/yellow]")
                                post_ward_retry_needed = True
                            else:
                                console.print(f"{indent}[bold red]{S.WARN}  Max post-ward retry attempts reached[/bold red]")

                        elif ward_result["mode"] == "advisory":
                            # Log warning but continue
                            console.print(f"{indent}  [yellow]{S.INFO}  Advisory notice (not blocking)[/yellow]")
        ...
        if cell.output_extraction:
            ...
            console.print(f"{indent}[bold cyan]{S.SEARCH} Extracting structured content...[/bold cyan]")
            ...
            try:
                ...
                if extracted is not None:
                    ...
                    console.print(f"{indent}  [green][OK] Extracted '{state_key}': {str(extracted)[:100]}...[/green]")
                    ...
                    console.print(f"{indent}  [yellow]{S.WARN}  Pattern not found (optional)[/yellow]")
...
def run_cascade(config_path: str | dict, input_data: dict | None = None, session_id: str = "default", overrides: dict | None = None,
                depth: int = 0, parent_trace: TraceNode | None = None, hooks: LARSHooks | None = None, parent_session_id: str | None = None,
    ...
    runner = LARSRunner(config_path, session_id, overrides, depth, parent_trace, hooks, take_index=take_index,
