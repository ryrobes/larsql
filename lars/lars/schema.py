...
CREATE TABLE IF NOT EXISTS unified_logs (
    ...
    udf_type LowCardinality(Nullable(String)),  -- 'lars_udf', 'lars_cascade_udf', 'llm_aggregate', 'semantic_op'
    ...
    cascade_version Nullable(UInt64),  -- Links to artifact_registry.version for audit trail
...
#
# Controlled by: LARS_SHADOW_ASSESSMENT_ENABLED (default: true)
...
CREATE TABLE IF NOT EXISTS ui_sql_log (
    ...
    -- Context
    source LowCardinality(String) DEFAULT 'unknown',  -- 'ui_backend', 'lars_core', etc.
...
# =============================================================================
# RUNTIME EVENT LOG (Append-Only) - Console/Server events
# =============================================================================
...
CREATE TABLE IF NOT EXISTS runtime_event_log (
    ...
    INDEX idx_connection connection_id TYPE bloom_filter GRANULARITY 1,
    INDEX idx_session session_id TYPE bloom_filter GRANULARITY 1,
...
CREATE TABLE IF NOT EXISTS sql_query_log (
    ...
    query_type LowCardinality(String),  -- 'lars_udf', 'lars_cascade_udf', 'lars_map', 'llm_aggregate', 'semantic_op'
...
# =============================================================================
# ARTIFACT REGISTRY - Versioned storage for cascades, skills, validators
# =============================================================================
# Unified registry for all LARS artifacts with version history and distributed sync.
# This enables:
# - Pre-parsed caching (no YAML parsing on every load)
# - Distributed consistency (all instances sync via DB)
...
ARTIFACT_REGISTRY_CURRENT_VIEW = """
...
# =============================================================================
# CASCADE CONFLICTS - Distributed sync conflict log
# =============================================================================
# Tracks conflicts when multiple instances modify the same artifact concurrently.

CASCADE_CONFLICTS_SCHEMA = """
...
def get_all_schemas() -> dict:
    ...
    return {
        ...
        "runtime_event_log": RUNTIME_EVENT_LOG_SCHEMA,
        ...
        "cascade_conflicts": CASCADE_CONFLICTS_SCHEMA,
    }


def get_views() -> dict:
    """
    Get all regular view schemas.

    Returns:
        Dict mapping view names to CREATE VIEW statements
    """
    return {
        "artifact_registry_current": ARTIFACT_REGISTRY_CURRENT_VIEW,
    }
...
def get_all_ddl() -> list:
    """
    Get all DDL statements in order (tables first, then views, then MVs).

    Returns:
        List of DDL statements
    """
    ddl = list(get_all_schemas().values())
    ddl.extend(get_views().values())
