...
Discovers:
- Python functions registered in skills registry
...
_skills_manifest_cache: Dict[str, Any] | None = None
_declarative_tools_registered: bool = False
_mcp_tools_registered: bool = False

def get_skill_manifest(refresh: bool = False) -> Dict[str, Any]:
    """
    Get unified manifest of all available skills (tools).
    ...
    """
    global _skills_manifest_cache, _declarative_tools_registered
    ...
    for skills_dir in all_skills_dirs:
        ...
        if not os.path.isabs(skills_dir):
            ...
            if not os.path.exists(search_path):
                # Try relative to lars package (for installed package)
        ...
        for ext in ('json', 'yaml', 'yml'):
            ...
            for cascade_path in glob.glob(pattern, recursive=True):
                try:
                    ...
                    if not get_skill(cascade_config.cascade_id):
                        try:
    ...
    _skills_manifest_cache = manifest
    return manifest
...
def format_manifest_for_quartermaster(manifest: Dict[str, Any]) -> str:
    ...
    for name, info in sorted(manifest.items()):
        skills_type = info["type"]
        description = info["description"].split("\n")[0]  # First line only
        lines.append(f"- {name} ({skills_type}): {description}")
