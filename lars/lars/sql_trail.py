...
# Known LARS UDF function names (lowercase for matching)
LARS_UDF_NAMES = {
    'lars_udf', 'lars', 'lars_cascade_udf', 'lars_run',
    'lars_run_batch', 'lars_run_parallel_batch', 'lars_map_parallel_exec',
...
def fingerprint_query(sql: str) -> Tuple[str, str, List[str]]:
    ...
    Returns:
        ...
        - udf_types: List of LARS UDF types found (e.g., ['lars_udf', 'llm_summarize'])
    """
...
def _extract_semantic_operators(sql: str) -> List[str]:
    ...
    try:
        from lars.sql_tools.dynamic_operators import get_operator_patterns_cached
        from .semantic_sql.registry import get_sql_function_registry
...
def _extract_udf_types_ast(ast: 'exp.Expression') -> List[str]:
    """Extract LARS UDF function calls from parsed AST."""
    udf_types = set()
...
def _determine_query_type(udf_types: List[str], sql: str) -> str:
    ...
    # Check for LARS MAP/RUN syntax
    if 'LARS MAP' in sql_upper:
        return 'lars_map'
    if 'LARS RUN' in sql_upper:
        return 'lars_run'

    # Check for specific UDF patterns
    if 'lars_cascade_udf' in udf_types or 'lars_run' in udf_types:
        return 'lars_cascade_udf'
    if 'lars_run_parallel_batch' in udf_types or 'lars_map_parallel_exec' in udf_types:
        return 'lars_map'
    if 'lars_udf' in udf_types or 'lars' in udf_types:
        return 'lars_udf'
...
def log_query_complete(
    ...
    try:
        ...
        # Result location columns (for auto-materialized LARS query results)
