...
# Find frontend build directory - check multiple locations
def _find_frontend_build():
    """Find the frontend build directory, checking multiple locations."""
    # 1. Environment variable override (highest priority)
    env_dir = os.environ.get('LARS_STUDIO_FRONTEND_BUILD')
    if env_dir and os.path.exists(os.path.join(env_dir, 'index.html')):
        return env_dir

    backend_dir = os.path.dirname(__file__)

    # 2. Package bundled location: ../frontend_build (same level as backend in package)
    bundled_dir = os.path.abspath(os.path.join(backend_dir, '..', 'frontend_build'))
    if os.path.exists(os.path.join(bundled_dir, 'index.html')):
        return bundled_dir

    # 3. Standard repo location: ../frontend/build
    repo_dir = os.path.abspath(os.path.join(backend_dir, '..', 'frontend', 'build'))
    if os.path.exists(os.path.join(repo_dir, 'index.html')):
        return repo_dir

    return repo_dir  # Return default even if not found
