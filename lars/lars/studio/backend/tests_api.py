...
def execute_tests(tests: List[TestDefinition], run_id: str, options: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if gevent_active:
        print(f"[TestsAPI] Detected gevent worker - running tests synchronously (no thread pool)")
        num_workers = 1
    else:
    ...
    # When running under gevent, skip ThreadPoolExecutor entirely and run synchronously
    if gevent_active:
        for test, mode in test_executions:
            ...
            print(f"[TestsAPI] {status_icon} [{completed}/{len(test_executions)}] {result.test_id} {mode_label} ({result.duration_ms:.0f}ms)", flush=True)
            if result.status in ('error', 'timed_out') and result.error_message:
                print(f"[TestsAPI]   {result.status}: {result.error_message[:100]}", flush=True)
        ...
        try:
            ...
            print(f"[TestsAPI] ❌ EXECUTOR ERROR: {type(outer_e).__name__}: {outer_e}", flush=True)
            traceback.print_exc()

    print(f"[TestsAPI] Test loop finished. Completed: {completed}, Passed: {passed_count}, Failed: {failed_count}, Errors: {error_count}", flush=True)
    ...
    try:
        ...
        for row in result_rows:
            ...
            result = {
                ...
                'source_line': _json_safe(row.get('source_line')),
                'duration_ms': _json_safe(row.get('duration_ms')),
