#!/usr/bin/env python3
"""
Glass Text Editor - A simple text editor with glass morphism effects
===================================================================

A custom text editor that works with our absolute positioning and glass
effects system, avoiding the issues with native Textual widgets.
"""

from .looking_glass import AbsoluteGlassMixin, BlazingFastBlendWidget, AbsoluteGlassPanel
from textual.strip import Strip, Segment
from rich.style import Style as RichStyle
from typing import List, Optional, Tuple
import re


class GlassTextEditor(AbsoluteGlassPanel):
    """
    A simple text editor with glass morphism effects.
    
    Features:
    - Multi-line text editing
    - Cursor movement with arrow keys
    - Basic text insertion and deletion
    - Maintains glass transparency
    """
    
    def __init__(self, initial_text: str = "", *args, **kwargs):
        """Initialize the text editor."""
        # Text buffer - list of lines
        self._lines = initial_text.split('\n') if initial_text else ['']
        self._cursor_row = 0
        self._cursor_col = 0
        self._scroll_offset_y = 0
        self._has_focus = False
        
        # Initialize parent with empty content
        super().__init__(content='', *args, **kwargs)
    
    def get_viewport_height(self) -> int:
        """Get the number of visible lines."""
        if hasattr(self, 'size') and self.size:
            return max(1, self.size.height - (4 if self.border else 0))
        return 10
    
    def get_viewport_width(self) -> int:
        """Get the number of visible columns."""
        if hasattr(self, 'size') and self.size:
            return max(1, self.size.width - (2 if self.border else 0))
        return 40
    
    def insert_char(self, char: str):
        """Insert a character at the cursor position."""
        if self._cursor_row < len(self._lines):
            line = self._lines[self._cursor_row]
            self._lines[self._cursor_row] = (
                line[:self._cursor_col] + char + line[self._cursor_col:]
            )
            self._cursor_col += 1
            self.refresh()
    
    def insert_newline(self):
        """Insert a newline at the cursor position."""
        if self._cursor_row < len(self._lines):
            line = self._lines[self._cursor_row]
            # Split the current line
            current_line = line[:self._cursor_col]
            next_line = line[self._cursor_col:]
            
            self._lines[self._cursor_row] = current_line
            self._lines.insert(self._cursor_row + 1, next_line)
            
            self._cursor_row += 1
            self._cursor_col = 0
            self._adjust_scroll()
            self.refresh()
    
    def delete_char(self):
        """Delete character before cursor (backspace)."""
        if self._cursor_col > 0:
            line = self._lines[self._cursor_row]
            self._lines[self._cursor_row] = (
                line[:self._cursor_col-1] + line[self._cursor_col:]
            )
            self._cursor_col -= 1
        elif self._cursor_row > 0:
            # Join with previous line
            prev_line = self._lines[self._cursor_row - 1]
            curr_line = self._lines[self._cursor_row]
            self._cursor_col = len(prev_line)
            self._lines[self._cursor_row - 1] = prev_line + curr_line
            self._lines.pop(self._cursor_row)
            self._cursor_row -= 1
            self._adjust_scroll()
        self.refresh()
    
    def move_cursor(self, direction: str):
        """Move cursor in the specified direction."""
        if direction == 'up' and self._cursor_row > 0:
            self._cursor_row -= 1
            self._cursor_col = min(self._cursor_col, len(self._lines[self._cursor_row]))
            self._adjust_scroll()
        elif direction == 'down' and self._cursor_row < len(self._lines) - 1:
            self._cursor_row += 1
            self._cursor_col = min(self._cursor_col, len(self._lines[self._cursor_row]))
            self._adjust_scroll()
        elif direction == 'left':
            if self._cursor_col > 0:
                self._cursor_col -= 1
            elif self._cursor_row > 0:
                self._cursor_row -= 1
                self._cursor_col = len(self._lines[self._cursor_row])
                self._adjust_scroll()
        elif direction == 'right':
            if self._cursor_col < len(self._lines[self._cursor_row]):
                self._cursor_col += 1
            elif self._cursor_row < len(self._lines) - 1:
                self._cursor_row += 1
                self._cursor_col = 0
                self._adjust_scroll()
        self.refresh()
    
    def _adjust_scroll(self):
        """Adjust scroll offset to keep cursor visible."""
        viewport_height = self.get_viewport_height()
        
        # Scroll up if cursor is above viewport
        if self._cursor_row < self._scroll_offset_y:
            self._scroll_offset_y = self._cursor_row
        
        # Scroll down if cursor is below viewport
        elif self._cursor_row >= self._scroll_offset_y + viewport_height:
            self._scroll_offset_y = self._cursor_row - viewport_height + 1
    
    def set_focus(self, focused: bool):
        """Set focus state."""
        self._has_focus = focused
        self.refresh()
    
    def get_text(self) -> str:
        """Get the full text content."""
        return '\n'.join(self._lines)
    
    def set_text(self, text: str):
        """Set the text content."""
        self._lines = text.split('\n') if text else ['']
        self._cursor_row = 0
        self._cursor_col = 0
        self._scroll_offset_y = 0
        self.refresh()
    
    def render_line(self, y: int) -> Strip:
        """Render a line with cursor and glass effects."""
        # Handle title and border
        if self.border:
            if y == 0:
                return super().render_line(y)
            elif y == 1:
                return super().render_line(y)
            elif y == self.size.height - 1:
                return super().render_line(y)
            # Add simple status line at bottom
            elif y == self.size.height - 2:
                status = f" {self._cursor_row + 1}:{self._cursor_col + 1} | Simple Editor "
                segments = []
                width = self.size.width - (2 if self.border else 0)
                
                # Simple status bar with dim style
                for i, char in enumerate(status[:width]):
                    segments.append(Segment(char, RichStyle(color="#808080", bgcolor=self.overlay_color)))
                # Pad the rest
                for i in range(len(status), width):
                    segments.append(Segment(" ", RichStyle(color="#808080", bgcolor=self.overlay_color)))
                
                return Strip(segments)
            else:
                content_y = y - 2
        else:
            content_y = y
        
        # Calculate which line to show
        line_index = content_y + self._scroll_offset_y
        
        # Get the line content
        if line_index < len(self._lines):
            line_text = self._lines[line_index]
        else:
            line_text = ""
        
        # Check if cursor is on this line
        cursor_on_line = (self._has_focus and 
                         line_index == self._cursor_row and 
                         content_y < self.get_viewport_height())
        
        # Render with glass effect
        width = self.size.width - (2 if self.border else 0)
        segments = []
        
        if self._precomputed_colors is not None and y < self._precomputed_colors.shape[0]:
            for x in range(width):
                # Get character
                char = line_text[x] if x < len(line_text) else ' '
                
                # Check if this is cursor position
                is_cursor = cursor_on_line and x == self._cursor_col
                
                # Get glass color
                actual_x = x + (1 if self.border else 0)
                if actual_x < self._precomputed_colors.shape[1]:
                    color = self._precomputed_colors[y, actual_x]
                    bg_hex = f"#{color[0]:02x}{color[1]:02x}{color[2]:02x}"
                else:
                    bg_hex = self.overlay_color
                
                # Create style
                if is_cursor:
                    style = RichStyle(bgcolor="white", color=bg_hex, bold=True)
                else:
                    style = RichStyle(bgcolor=bg_hex, color="white", bold=True)
                
                segments.append(Segment(char, style))
        else:
            # Fallback rendering
            for x in range(width):
                char = line_text[x] if x < len(line_text) else ' '
                is_cursor = cursor_on_line and x == self._cursor_col
                
                if is_cursor:
                    style = RichStyle(bgcolor="white", color=self.overlay_color, bold=True)
                else:
                    style = RichStyle(bgcolor=self.overlay_color, color="white", bold=True)
                
                segments.append(Segment(char, style))
        
        return Strip(segments)
    
    def on_key(self, event) -> None:
        """Handle key events."""
        # Only process keys if we have focus
        if not self._has_focus:
            return
            
        key = event.key
        
        # Save shortcut
        if key == "ctrl+s":
            # Trigger save - need to notify parent app
            if hasattr(self.app, 'dispatch'):
                content = self.get_text()
                from looking_glass_reactive import Action
                self.app.dispatch(Action('UPDATE_SQL_CONTENT', content))
            return
            
        # Navigation
        if key == "up":
            self.move_cursor("up")
        elif key == "down":
            self.move_cursor("down")
        elif key == "left":
            self.move_cursor("left")
        elif key == "right":
            self.move_cursor("right")
        elif key == "home":
            self._cursor_col = 0
            self.refresh()
        elif key == "end":
            if self._cursor_row < len(self._lines):
                self._cursor_col = len(self._lines[self._cursor_row])
                self.refresh()
        
        # Editing
        elif key == "enter":
            self.insert_newline()
        elif key == "backspace":
            self.delete_char()
        elif key == "delete":
            # Delete forward
            if self._cursor_col < len(self._lines[self._cursor_row]):
                self._cursor_col += 1
                self.delete_char()
            elif self._cursor_row < len(self._lines) - 1:
                # Join with next line
                self._cursor_row += 1
                self._cursor_col = 0
                self.delete_char()
        elif key == "tab":
            self.insert_char("    ")  # 4 spaces
        elif key == "space":
            self.insert_char(" ")
            
        # Text input - handle printable characters and special chars
        elif hasattr(event, 'character') and event.character:
            self.insert_char(event.character)
        elif len(key) == 1:
            self.insert_char(key)
    
    def on_mount(self):
        """Request focus when mounted."""
        # Glass widgets don't need to call super().on_mount()
        self.set_focus(True)
        # Debug positioning
        if hasattr(self, 'abs_x') and hasattr(self, 'abs_y'):
            self.log(f"GlassTextEditor mounted at abs position: ({self.abs_x}, {self.abs_y})")
        if hasattr(self, 'region'):
            self.log(f"GlassTextEditor region: {self.region}")
        
    def on_click(self, event) -> None:
        """Handle mouse clicks to set focus."""
        self.set_focus(True)