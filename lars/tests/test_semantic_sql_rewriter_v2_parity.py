...
def test_v2_enabled_produces_expected_rewrite(query: str, must_contain: str, monkeypatch):
    monkeypatch.setenv("LARS_SEMANTIC_REWRITE_V2", "1")
...
def test_v2_parity_with_legacy_for_common_queries(monkeypatch):
    ...
    monkeypatch.delenv("LARS_SEMANTIC_REWRITE_V2", raising=False)
    legacy = [rewrite_lars_syntax(q) for q in queries]

    monkeypatch.setenv("LARS_SEMANTIC_REWRITE_V2", "1")
    v2 = [rewrite_lars_syntax(q) for q in queries]
