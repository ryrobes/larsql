...
Tests LARS MAP/RUN syntax parsing and rewriting.
"""
...
def test_detect_lars_map():
    """Should detect LARS MAP statements."""
    assert _is_lars_statement("LARS MAP 'x' USING (SELECT 1)")
    assert _is_lars_statement("lars map 'x' using (select 1)")  # case-insensitive
    assert _is_lars_statement("  LARS MAP 'x' USING (SELECT 1)  ")  # whitespace


def test_detect_lars_run():
    """Should detect LARS RUN statements."""
    assert _is_lars_statement("LARS RUN 'x' USING (SELECT 1)")


def test_not_lars_statement():
    """Should not detect regular SQL."""
    assert not _is_lars_statement("SELECT * FROM t")
    assert not _is_lars_statement("INSERT INTO t VALUES (1)")
    assert not _is_lars_statement("-- LARS MAP 'x' USING (SELECT 1)")  # comment


def test_detect_semantic_sql_newer_operators_for_logging():
    """Should detect newer semantic operators so SQL Trail logging runs."""
    # Function-style (short alias): aligns(...)
    assert _is_lars_statement("SELECT ALIGNS(text, 'SQL is good for AI') as score FROM docs")
    # Infix style: ALIGNS WITH
    assert _is_lars_statement("SELECT * FROM policies WHERE description ALIGNS WITH 'customer-first values'")
    # ASK infix operator
    assert _is_lars_statement("SELECT body ASK 'summarize in 5 words' FROM emails")
    # Vector search (explicit DuckDB form)
    assert _is_lars_statement("SELECT * FROM read_json_auto(vector_search_json_3('eco', 'products', 5))")
...
def test_parse_basic_map():
    ...
    stmt = _parse_lars_statement(
        "LARS MAP 'x.yaml' USING (SELECT a FROM t)"
    )
...
def test_parse_map_with_as_alias():
    ...
    stmt = _parse_lars_statement(
        "LARS MAP 'x.yaml' AS enriched USING (SELECT a FROM t)"
    )
...
def test_parse_map_with_parallel():
    ...
    stmt = _parse_lars_statement(
        "LARS MAP PARALLEL 5 'x.yaml' USING (SELECT a FROM t LIMIT 10)"
    )
    assert stmt.mode == 'MAP'
...
def test_parse_map_parallel_with_alias():
    ...
    stmt = _parse_lars_statement(
        "LARS MAP PARALLEL 20 'x.yaml' AS enriched USING (SELECT a FROM t)"
    )
...
def test_parse_error_missing_cascade():
    ...
    with pytest.raises(LARSSyntaxError, match="Expected cascade path"):
        _parse_lars_statement("LARS MAP USING (SELECT 1)")
...
def test_parse_error_missing_using():
    ...
    with pytest.raises(LARSSyntaxError, match="Expected USING"):
        _parse_lars_statement("LARS MAP 'x.yaml'")
...
def test_parse_error_unbalanced_parens():
    ...
    with pytest.raises(LARSSyntaxError, match="Unbalanced parentheses"):
        _parse_lars_statement("LARS MAP 'x' USING (SELECT a FROM t")
...
def test_rewrite_map_basic():
    ...
    stmt = _parse_lars_statement(
        "LARS MAP 'x.yaml' USING (SELECT a FROM t LIMIT 10)"
    )
    rewritten = _rewrite_map(stmt)

    # Check structure
    assert 'WITH lars_input AS' in rewritten
    assert 'lars_raw AS' in rewritten
    assert "SELECT a FROM t LIMIT 10" in rewritten
    assert "lars_run('x.yaml'" in rewritten
...
def test_rewrite_map_with_alias():
    ...
    stmt = _parse_lars_statement(
        "LARS MAP 'x.yaml' AS enriched USING (SELECT a FROM t LIMIT 10)"
    )
    rewritten = _rewrite_map(stmt)
...
def test_rewrite_lars_syntax_passthrough():
    """Should pass through non-LARS queries unchanged."""
    query = "SELECT * FROM products LIMIT 10"
    result = rewrite_lars_syntax(query)
    assert result == query


def test_rewrite_lars_syntax_map():
    """Should rewrite LARS MAP."""
    query = "LARS MAP 'enrich.yaml' AS enriched USING (SELECT * FROM t LIMIT 10)"
    result = rewrite_lars_syntax(query)

    assert 'WITH lars_input AS' in result
    assert 'lars_raw AS' in result
    assert 'SELECT * FROM t LIMIT 10' in result
    assert "lars_run('enrich.yaml'" in result
...
def test_parse_basic_run():
    ...
    stmt = _parse_lars_statement(
        "LARS RUN 'batch.yaml' USING (SELECT * FROM t LIMIT 500)"
    )
...
def test_rewrite_run_basic():
    ...
    stmt = _parse_lars_statement(
        "LARS RUN 'batch.yaml' USING (SELECT a, b FROM t LIMIT 100) WITH (as_table = 'batch_data')"
    )
    rewritten = _rewrite_run(stmt)

    assert 'lars_run_batch(' in rewritten
...
def test_rewrite_run_auto_table_name():
    ...
    stmt = _parse_lars_statement(
        "LARS RUN 'batch.yaml' USING (SELECT * FROM t)"
    )
    rewritten = _rewrite_run(stmt)

    assert 'lars_run_batch(' in rewritten
    assert "'_lars_batch_" in rewritten  # Auto-generated name


def test_rewrite_lars_syntax_run():
    """Should rewrite LARS RUN end-to-end."""
    query = "LARS RUN 'batch.yaml' USING (SELECT * FROM t LIMIT 100) WITH (as_table = 'data')"
    result = rewrite_lars_syntax(query)

    assert 'lars_run_batch(' in result
...
def test_example_fraud_detection():
    ...
    query = """
        LARS MAP 'cascades/fraud_assess.yaml' AS fraud_risk
        USING (
    ...
    assert "lars_run('cascades/fraud_assess.yaml'" in result
    assert 'COALESCE(' in result
...
def test_example_product_enrichment():
    ...
    query = """
        LARS MAP 'skills/extract_brand.yaml'
        USING (
    ...
    assert "lars_run('skills/extract_brand.yaml'" in result
...
def test_example_with_complex_join():
    ...
    query = """
        LARS MAP 'analyze.yaml' AS analysis
        USING (
            SELECT
                c.customer_id,
                c.name,
    ...
    assert "lars_run('analyze.yaml'" in result
...
def test_error_invalid_syntax():
    """Should pass through unrecognized LARS keywords (will fail at DuckDB)."""
    # LARS INVALID doesn't match MAP or RUN, so it passes through
    result = rewrite_lars_syntax("LARS INVALID 'x' USING (SELECT 1)")
    assert result == "LARS INVALID 'x' USING (SELECT 1)"


def test_error_missing_quote():
    """Should raise error for missing quote in cascade path."""
    with pytest.raises(LARSSyntaxError, match="Expected cascade path"):
        rewrite_lars_syntax("LARS MAP x.yaml USING (SELECT 1)")


def test_error_with_malformed_options():
    """Should raise error for malformed WITH options."""
    with pytest.raises(LARSSyntaxError, match="key = value"):
        rewrite_lars_syntax("""
            LARS MAP 'x' USING (SELECT 1) WITH (invalid_syntax)
        """)
...
def test_arrow_rewrite_injects_hint():
    """Should inject LARS hint comment for arrow alias."""
    result = rewrite_lars_syntax('SELECT * FROM emails -> players;')
    assert '/*LARS:save_as=players*/' in result
    assert '->' not in result


def test_arrow_rewrite_dotted_hint():
    """Should inject hint for dotted table names."""
    result = rewrite_lars_syntax('SELECT * FROM emails -> enron.suspects;')
    assert '/*LARS:save_as=enron.suspects*/' in result


def test_arrow_rewrite_preserves_query():
    """Should preserve the query content after removing arrow."""
    result = rewrite_lars_syntax('SELECT a, b, c FROM my_table WHERE x = 1 -> output;')
    assert 'SELECT a, b, c FROM my_table WHERE x = 1' in result
    assert '/*LARS:save_as=output*/' in result


def test_arrow_no_hint_without_arrow():
    """Should not inject hint when no arrow present."""
    result = rewrite_lars_syntax('SELECT * FROM emails')
    assert '/*LARS:' not in result
...
def test_shadow_as_rewrite_injects_hint():
    """Should inject LARS hint comment for SHADOW AS alias."""
    result = rewrite_lars_syntax('SELECT * FROM emails SHADOW AS players;')
    assert '/*LARS:save_as=players*/' in result
...
def test_shadow_as_rewrite_preserves_query():
    ...
    result = rewrite_lars_syntax('SELECT a, b FROM t WHERE x = 1 SHADOW AS output;')
