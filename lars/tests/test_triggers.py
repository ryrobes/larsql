...
def test_cron_trigger_model():
    ...
    from lars.cascade import CronTrigger
...
def test_webhook_trigger_model():
    ...
    from lars.cascade import WebhookTrigger
...
def test_manual_trigger_model():
    ...
    from lars.cascade import ManualTrigger
...
def test_cascade_with_triggers():
    ...
    from lars.cascade import CascadeConfig, CellConfig, CronTrigger, SensorTrigger
...
def test_list_triggers():
    ...
    from lars.cascade import CascadeConfig, CellConfig, CronTrigger

    config = CascadeConfig(
        cascade_id="test",
        cells=[CellConfig(name="p1", instructions="test")],
        triggers=[
...
def test_list_triggers_empty():
    ...
    from lars.triggers import list_triggers
    from lars.cascade import CascadeConfig, CellConfig
...
def test_get_trigger():
    ...
    from lars.triggers import get_trigger
    from lars.cascade import CascadeConfig, CellConfig, CronTrigger, SensorTrigger
...
def test_export_cron():
    ...
    from lars.triggers import export_cron
    from lars.cascade import CascadeConfig, CellConfig, CronTrigger
    ...
    assert "LARS triggers for test_cascade" in output
    assert "0 6 * * *" in output
    assert "lars run /path/to/cascade.json" in output
...
def test_export_kubernetes():
    ...
    from lars.triggers import export_kubernetes
    from lars.cascade import CascadeConfig, CellConfig, CronTrigger
...
def test_export_systemd():
    ...
    assert "User=lars" in service_content
    assert "ExecStart=" in service_content
    assert "lars run /opt/lars/cascade.json" in service_content


def test_export_airflow():
    """Test exporting to Airflow DAG format."""
    from lars.triggers import export_airflow
    from lars.cascade import CascadeConfig, CellConfig, CronTrigger
    ...
    assert 'dag_id="lars_test_cascade"' in output
...
def test_sensor_file_exists():
    ...
    from lars.triggers import sensor_file_exists
    import tempfile
    import os
...
def test_sensor_http_healthy():
    ...
    from lars.triggers import sensor_http_healthy
...
def test_cron_to_systemd_calendar():
    ...
    from lars.triggers import _cron_to_systemd_calendar
