...
Language = Literal["python", "sql", "clojure"]
...
class ScopedVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> ast.Call:
        ...
        # Handle mo.clj() Clojure calls - extract inputs/outputs for reactivity
        valid_clj_calls = ["marimo.clj", "mo.clj"]
        if (
            isinstance(node.func, ast.Attribute)
            and isinstance(node.func.value, ast.Name)
            and f"{node.func.value.id}.{node.func.attr}" in valid_clj_calls
        ):
            self.language = "clojure"

            # Check if auto=True is present
            auto_mode = False
            explicit_inputs: list[str] = []
            explicit_outputs: list[str] = []

            for kw in node.keywords:
                if kw.arg == "auto" and isinstance(kw.value, ast.Constant):
                    auto_mode = bool(kw.value.value)
                elif kw.arg == "inputs" and isinstance(kw.value, ast.List):
                    for elt in kw.value.elts:
                        if isinstance(elt, ast.Constant) and isinstance(
                            elt.value, str
                        ):
                            explicit_inputs.append(elt.value)
                elif kw.arg == "outputs" and isinstance(kw.value, ast.List):
                    for elt in kw.value.elts:
                        if isinstance(elt, ast.Constant) and isinstance(
                            elt.value, str
                        ):
                            explicit_outputs.append(elt.value)

            if auto_mode:
                # Auto-detect inputs/outputs from Clojure code using static analysis
                clojure_code = ""
                if node.args and isinstance(node.args[0], ast.Constant):
                    clojure_code = str(node.args[0].value)
                elif node.args and isinstance(node.args[0], ast.JoinedStr):
                    # Handle f-strings - extract the string parts
                    parts = []
                    for value in node.args[0].values:
                        if isinstance(value, ast.Constant):
                            parts.append(str(value.value))
                    clojure_code = "".join(parts)
                ...
                if clojure_code:
                    try:
                        ...
                        # Detect outputs (def/defn forms)
                        detected_outputs = detect_outputs(clojure_code)
                        all_outputs = set(explicit_outputs) | detected_outputs

                        # Detect potential inputs (referenced symbols)
                        # Note: We can't know available Python vars at compile time,
                        # so we detect all potential inputs and let runtime filter
                        detected_inputs = detect_inputs(clojure_code, None)
                        all_inputs = set(explicit_inputs) | detected_inputs

                        # Add detected inputs as refs
                        for inp in all_inputs:
                            self._add_ref(node, inp, deleted=False)

                        # Add detected outputs as defs
                        for out in all_outputs:
                            self._define(
                                node, out, VariableData(kind="variable")
                            )
                    except ImportError:
                        # Fallback to explicit only if analyzer not available
                ...
                # Explicit mode - use inputs/outputs as declared
                for inp in explicit_inputs:
                    self._add_ref(node, inp, deleted=False)
                for out in explicit_outputs:
                    self._define(node, out, VariableData(kind="variable"))
