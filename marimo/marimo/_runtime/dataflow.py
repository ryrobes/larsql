...
class DirectedGraph:
    ...
    def get_referring_cells(
        self, name: Name, language: Literal["python", "sql", "clojure"]
    ) -> set[CellId_t]:
        """Get all cells that have a ref to `name`.

        The variable can be either a Python variable, a SQL variable (table),
        or a Clojure variable. SQL variables don't leak to Python cells, but
        Python and Clojure variables do leak to all other cells.
