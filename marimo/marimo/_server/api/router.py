...
def build_routes(base_url: str = "") -> list[BaseRoute]:
    ...
    # DEV FORK: Enable dev endpoints when MARIMO_DEV_MODE is set
    # These endpoints expose kernel internals for external tool integration.
    # ONLY enabled when MARIMO_DEV_MODE environment variable is set.
    if os.environ.get("MARIMO_DEV_MODE", "").lower() in ("1", "true", "yes"):
        from starlette.routing import Route, WebSocketRoute

        from marimo._server.api.endpoints.dev_stream import router as dev_router

        # Add routes directly with prefix, inserted at START to avoid
        # being caught by the catch-all Mount at "" (health router)
        prefix = "/api/dev"
        insert_position = 0
        for route in dev_router.routes:
            if hasattr(route, "path"):
                new_path = prefix + route.path
                # Handle WebSocket routes vs regular HTTP routes
                ...
                app_router.routes.insert(insert_position, new_route)
                insert_position += 1
