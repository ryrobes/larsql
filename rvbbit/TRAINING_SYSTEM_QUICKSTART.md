...
clickhouse-client --host localhost --query "$(cat rvbbit/migrations/create_universal_training_system.sql)"
```
...
```python
# Or via Python
from rvbbit.training_system import get_training_stats
...
rvbbit run test_classifier_with_training.yaml --input '{"text": "This is amazing!"}' --session test1

# Mark as trainable
# (Get trace_id from unified_logs, then mark it)

# Second run (uses first as training example!)
rvbbit run test_classifier_with_training.yaml --input '{"text": "This is fantastic!"}' --session test2
```
...
3. Check logs: `rvbbit sql query "SELECT * FROM training_examples_with_annotations LIMIT 5"`
