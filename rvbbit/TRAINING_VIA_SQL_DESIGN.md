# RVBBIT "Training" via SQL-Injected Few-Shot Learning
...
CREATE TABLE rvbbit_training_examples (
    id UUID PRIMARY KEY,
...
CREATE INDEX idx_training_operator ON rvbbit_training_examples(operator);
CREATE INDEX idx_training_confidence ON rvbbit_training_examples(confidence DESC);
```

**Example data:**
```sql
INSERT INTO rvbbit_training_examples VALUES
...
INSERT INTO rvbbit_training_examples (operator, input_text, output_value, confidence, metadata, created_by)
VALUES
...
- Auto-populates `rvbbit_training_examples` table
...
SET rvbbit.training_examples = 5;
SET rvbbit.training_strategy = 'semantic_similarity';

-- Queries now automatically use training examples
SELECT * FROM products WHERE description MEANS 'eco-friendly';

-- Disable
SET rvbbit.training_examples = 0;
```
...
training:
  ...
  table: rvbbit_training_examples
```
...
| Aspect | PostgresML Fine-Tuning | RVBBIT Few-Shot SQL |
|--------|------------------------|---------------------|
...
**Winner: RVBBIT Few-Shot SQL** for most semantic SQL use cases.
...
### Practical Implications for RVBBIT
...
```python
# rvbbit/traits/training_examples.py

from rvbbit import register_tackle
from rvbbit.db_adapter import get_clickhouse_client
...
def get_training_examples(operator: str, query_text: str = None, limit: int = 5, strategy: str = 'random'):
    ...
    if strategy == 'semantic' and query_text:
        # Embed query
        from rvbbit.traits.embedding_tools import agent_embed
        query_embedding = agent_embed(query_text)
...
def add_training_example(operator: str, input_text: str, output_value: str,
    ...
    client.execute("""
        INSERT INTO rvbbit_training_examples
...
CREATE TABLE IF NOT EXISTS rvbbit_training_examples (
    id String,
...
CREATE INDEX idx_operator ON rvbbit_training_examples(operator) TYPE set(0);
CREATE INDEX idx_confidence ON rvbbit_training_examples(confidence) TYPE minmax;
```
...
def execute_sql_function(name, args, session_id=None):
    ...
    if training_config.get('enabled'):
        ...
        # Retrieve examples
        from rvbbit.traits.training_examples import get_training_examples
    ...
    from rvbbit.semantic_sql.executor import execute_cascade_udf
...
**RVBBIT:** Works with Claude Opus 4.5, GPT-4, Gemini Pro - models you **cannot** fine-tune
...
**RVBBIT:** Retrieves **only relevant examples** via semantic similarity
...
**RVBBIT:** See exact examples used in each query

```sql
-- View training examples used for a query
SELECT * FROM rvbbit_query_log
...
**RVBBIT:** Curated examples table (human verification per row)

```sql
-- Review all training examples for an operator
SELECT * FROM rvbbit_training_examples
WHERE operator = 'semantic_matches'
  AND confidence < 0.9
ORDER BY created_at DESC;

-- Remove bad examples
DELETE FROM rvbbit_training_examples WHERE id = 'uuid-bad-example';

-- Update example quality
UPDATE rvbbit_training_examples
...
5. **Performance Comparison:**
   - Benchmark: Few-shot RVBBIT vs. Fine-tuned PostgresML
...
- ✅ Create `rvbbit_training_examples` table migration
...
- CLI command: `rvbbit training add <operator> <input> <output>`
...
**Recommendation:** Implement Phase 1 MVP to validate the approach. This is a **genuine innovation** that makes RVBBIT's "training" system competitive with (and arguably better than) PostgresML's fine-tuning.
