"""
Alice TUI Dashboard Generator for RVBBIT Cascade Visualization

Generates declarative Alice YAML dashboards from RVBBIT cascade definitions,
enabling real-time TUI monitoring of cascade execution.

Usage:
    from rvbbit.alice_generator import generate_alice_yaml
...
def generate_cell_status_block(cell: CellInfo, cascade_id: str, session_id: Optional[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        'type': 'continuous',
        'visible': False,
        'command': f'while true; do\nrvbbit sql query "{sql_query}" --format json 2>/dev/null || echo \'[{{"completed": 0, "started": 0, "cost": 0}}]\'\nsleep 1\ndone',
        'x': 0,
        'y': 0,
        'width': 1,
        'height': 1,
...
def generate_session_resolver_block(cascade_id: str, session_id: Optional[str], refresh_interval: float = 1.0) -> Dict[str, Any]:
    ...
    if session_id and session_id not in ('latest', '{{SESSION_ID}}', ''):
        ...
        command = f'''while true; do
rvbbit sql query "SELECT session_id, cascade_id FROM all_data WHERE cascade_id = '{cascade_id}' ORDER BY timestamp DESC LIMIT 1" --format json 2>/dev/null || echo '[{{"session_id": "", "cascade_id": ""}}]'
...
def generate_polling_block(cascade_id: str, session_id: Optional[str], refresh_interval: float = 0.5) -> Dict[str, Any]:
    ...
    return {
        ...
        'type': 'continuous',
        'visible': False,
        'command': f'while true; do\nrvbbit sql query "{sql_query}" --format json 2>/dev/null || echo \'[]\'\nsleep {refresh_interval}\ndone',
        'x': 0,
        'y': 0,
        'width': 1,
        'height': 1,
...
def generate_log_block(cascade_id: str, session_id: Optional[str], x: int, y: int, width: int, height: int) -> Dict[str, Any]:
    ...
    return {
        ...
        'type': 'continuous',
        'command': f'while true; do\nrvbbit sql query "{sql_query}" --format table 2>/dev/null || echo \'Waiting for data...\'\nsleep 0.5\ndone',
        'x': x,
        'y': y,
        'width': width,
        'height': height,
...
def generate_header_block(cascade_id: str, session_id: Optional[str], x: int, y: int, width: int) -> Dict[str, Any]:
    ...
    return {
        ...
        'height': 5,
        'content': [
            f"[bold magenta]RVBBIT Cascade Monitor[/bold magenta]",
            "",
...
def generate_cost_chart_block(cascade_id: str, session_id: Optional[str], x: int, y: int, width: int, height: int) -> Dict[str, Any]:
    ...
    return {
        'block_id': 'cost_chart',
        'type': 'continuous',
        'command': f'while true; do\nrvbbit sql query "{sql_query}" --format json 2>/dev/null | cheshire "SELECT cell_name as x, cost as y FROM data" bar 0 --title "Cost by Cell" 2>/dev/null || echo "Waiting for data..."\nsleep 2\ndone',
        'x': x,
        'y': y,
        'width': width,
        'height': height,
...
def generate_timeline_chart_block(cascade_id: str, session_id: Optional[str], x: int, y: int, width: int, height: int) -> Dict[str, Any]:
    ...
    return {
        ...
        'type': 'continuous',
        'command': f'while true; do\nrvbbit sql query "{sql_query}" --format json 2>/dev/null | cheshire "SELECT ts as x, cnt as y FROM data" line 0 --title "Activity" 2>/dev/null || echo "Waiting for data..."\nsleep 2\ndone',
        'x': x,
        'y': y,
        'width': width,
        'height': height,
