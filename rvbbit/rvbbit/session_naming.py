...
def get_session_id_style() -> str:
    ...
    Checks RVBBIT_SESSION_ID_STYLE (defaults to 'woodland')

    Returns:
        'woodland', 'uuid', or 'coolname'
    """
    import os
    style = os.getenv('RVBBIT_SESSION_ID_STYLE', 'woodland').lower()

    if style not in ('woodland', 'uuid', 'coolname'):
        print(f"[Warning] Invalid RVBBIT_SESSION_ID_STYLE='{style}', using 'woodland'")
        return 'woodland'
...
def auto_generate_session_id() -> str:
    ...
    Respects RVBBIT_SESSION_ID_STYLE env var (default: woodland)
