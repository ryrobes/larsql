...
class DatabaseConnector:
    ...
    def __init__(self, use_cache: bool = True):
        ...
        if use_cache:
            ...
            data_dir = os.getenv('RVBBIT_DATA_DIR', os.path.join(os.getcwd(), 'data'))
    ...
    def _attach_bigquery(self, config: SqlConnectionConfig, alias: str):
        ...
        print(f"    ✓ Attached BigQuery project: {config.project_id} → {alias}")
    ...
    def _attach_snowflake(self, config: SqlConnectionConfig, alias: str):
        ...
        print(f"    ✓ Attached Snowflake: {config.account} → {alias}")
    ...
    def _attach_motherduck(self, config: SqlConnectionConfig, alias: str):
        ...
        print(f"    ✓ Attached Motherduck: {config.database} → {alias}")
    ...
    def _attach_s3(self, config: SqlConnectionConfig, alias: str):
        ...
        for file_path in files:
            ...
            try:
                ...
                loaded_count += 1
                print(f"    ✓ Registered S3 file: {Path(file_path).name} → {alias}.{table_name}")
            except Exception as e:
                print(f"    ⚠️  Skipped {file_path}: {str(e)[:80]}")
    ...
    def _attach_azure(self, config: SqlConnectionConfig, alias: str):
        ...
        for file_path in files:
            ...
            try:
                ...
                loaded_count += 1
                print(f"    ✓ Registered Azure file: {Path(file_path).name} → {alias}.{table_name}")
            except Exception as e:
                print(f"    ⚠️  Skipped {file_path}: {str(e)[:80]}")
    ...
    def _attach_gcs(self, config: SqlConnectionConfig, alias: str):
        ...
        for file_path in files:
            ...
            try:
                ...
                loaded_count += 1
                print(f"    ✓ Registered GCS file: {Path(file_path).name} → {alias}.{table_name}")
            except Exception as e:
                print(f"    ⚠️  Skipped {file_path}: {str(e)[:80]}")
    ...
    def _attach_http(self, config: SqlConnectionConfig, alias: str):
        ...
        if not base_url:
            print(f"    ⚠️  HTTP connection {alias} missing folder_path")
            return
        ...
        try:
            ...
            print(f"    ✓ Attached HTTP file: {base_url} → {alias}.{table_name}")
        except Exception as e:
            print(f"    ⚠️  Failed to attach HTTP file {base_url}: {str(e)[:80]}")
    ...
    def _attach_delta(self, config: SqlConnectionConfig, alias: str):
        ...
        try:
            ...
            """)
            print(f"    ✓ Attached Delta table: {config.table_path} → {alias}.{table_name}")
        except Exception as e:
            print(f"    ⚠️  Failed to attach Delta table: {str(e)[:80]}")
    ...
    def _attach_iceberg(self, config: SqlConnectionConfig, alias: str):
        ...
        if config.catalog_type == "rest" and config.catalog_uri:
            ...
            try:
                ...
                print(f"    ✓ Attached Iceberg REST catalog: {config.catalog_uri} → {alias}")
            except Exception as e:
                print(f"    ⚠️  Failed to attach Iceberg catalog: {str(e)[:80]}")
            ...
            try:
                ...
                """)
                print(f"    ✓ Attached Iceberg table: {config.table_path} → {alias}.{table_name}")
            except Exception as e:
                print(f"    ⚠️  Failed to attach Iceberg table: {str(e)[:80]}")
    ...
    def _attach_odbc(self, config: SqlConnectionConfig, alias: str):
        ...
        if config.odbc_connection_string_env:
            ...
            print(f"    ⚠️  ODBC connection {alias} requires odbc_dsn or odbc_connection_string_env")
            return
        ...
        print(f"    ℹ️  ODBC connection configured: {alias}")
    ...
    def _attach_gsheets(self, config: SqlConnectionConfig, alias: str):
        ...
        try:
            ...
            print(f"    ⚠️  gsheets extension not available: {str(e)[:60]}")
        ...
        try:
            ...
            print(f"    ⚠️  Failed to load gsheets extension: {str(e)[:60]}")
            return
        ...
        try:
            ...
            print(f"    ✓ Attached Google Sheet: {config.spreadsheet_id} → {alias}.{table_name}")
        except Exception as e:
            print(f"    ⚠️  Failed to attach Google Sheet: {str(e)[:80]}")
    ...
    def _attach_excel(self, config: SqlConnectionConfig, alias: str):
        ...
        if not config.file_path:
            print(f"    ⚠️  Excel connection {alias} requires file_path")
            return

        file_path = Path(config.file_path)
        if not file_path.exists():
            print(f"    ⚠️  Excel file not found: {config.file_path}")
            return
        ...
        for sheet in sheets:
            ...
            try:
                ...
                """)
                loaded_count += 1
                print(f"    ✓ Attached Excel sheet: {sheet} → {alias}.{table_name}")
            except Exception as e:
                ...
                try:
                    ...
                    print(f"    ⚠️  Failed to attach sheet {sheet}: {str(e2)[:60]}")
    ...
    def _attach_mongodb(self, config: SqlConnectionConfig, alias: str):
        ...
        try:
            ...
            print(f"    ⚠️  MongoDB connector requires: pip install pymongo pandas")
            return

        uri = os.getenv(config.mongodb_uri_env) if config.mongodb_uri_env else None
        if not uri:
            print(f"    ⚠️  MongoDB connection {alias} missing mongodb_uri_env or env var not set")
            return

        if not config.database:
            print(f"    ⚠️  MongoDB connection {alias} missing database name")
            return
        ...
        try:
            ...
            for collection_name in db.list_collection_names():
                ...
                self.conn.execute(f"""
                    CREATE TABLE {alias}.{table_name} AS
                ...
                collection_count += 1
                total_rows += len(df)
                print(f"    ✓ Materialized collection: {collection_name} → {alias}.{table_name} ({len(df)} rows)")

            client.close()
            print(f"  └─ Materialized MongoDB: {config.database} ({collection_count} collections, {total_rows:,} rows)")

        except Exception as e:
            print(f"    ⚠️  Failed to connect to MongoDB: {str(e)[:80]}")
    ...
    def _attach_cassandra(self, config: SqlConnectionConfig, alias: str):
        ...
        try:
            ...
            print(f"    ⚠️  Cassandra connector requires: pip install cassandra-driver pandas")
            return

        if not config.cassandra_hosts:
            print(f"    ⚠️  Cassandra connection {alias} missing cassandra_hosts")
            return

        if not config.cassandra_keyspace:
            print(f"    ⚠️  Cassandra connection {alias} missing cassandra_keyspace")
            return
        ...
        try:
            ...
            for row in tables:
                ...
                try:
                    ...
                    table_count += 1
                    total_rows += len(df)
                    print(f"    ✓ Materialized table: {table_name} → {alias}.{table_name} ({len(df)} rows)")
            ...
            print(f"    ⚠️  Failed to connect to Cassandra: {str(e)[:80]}")
    ...
    def _attach_clickhouse(self, config: SqlConnectionConfig, alias: str):
        ...
        try:
            ...
            print(f"    ⚠️  ClickHouse connector requires: pip install clickhouse-connect pandas")
            return

        if not config.host:
            print(f"    ⚠️  ClickHouse connection {alias} missing host")
            return
        ...
        try:
            ...
            for table_name in table_names:
                ...
                try:
                    ...
                    self.conn.register(temp_name, df)
                    self.conn.execute(f"""
                        CREATE TABLE {alias}.{safe_table_name} AS
                        SELECT * FROM {temp_name}
                    """)
                    self.conn.unregister(temp_name)
                    ...
                    table_count += 1
                    total_rows += len(df)
                    print(f"    ✓ Materialized table: {table_name} → {alias}.{safe_table_name} ({len(df)} rows)")
            ...
            print(f"    ⚠️  Failed to connect to ClickHouse: {str(e)[:80]}")
    ...
    def _attach_duckdb_file(self, db_file: Path, db_name: str, max_retries: int = 2) -> bool:
        ...
        try:
            # Use a dedicated temp directory for rvbbit DuckDB snapshots
