...
Workflow:
1. Run cascade normally: rvbbit run my_flow.json --input {...} --session test_001
2. Verify it worked correctly
3. Freeze as test: rvbbit test freeze test_001 --name my_test --extract-all
4. Replay anytime: rvbbit test replay my_test --mode deterministic
"""
import json
import re
import hashlib
    ...
    def verify(self, routing_path: List[str]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if to_idx == from_idx + 1:
                return True, f"✓ Routing {self.from_cell} → {self.to_cell}"
            else:
...
class StateMutationContract:
    ...
    def verify(self, cell_state_changes: Dict[str, Dict[str, Any]]) -> Tuple[bool, str]:
        ...
        return True, f"✓ Cell '{self.cell}' set expected state keys"
...
class ToolCallContract:
    ...
    def verify(self, cell_tool_calls: Dict[str, List[Dict]]) -> Tuple[bool, str]:
        ...
        return True, f"✓ Cell '{self.cell}' called {self.tool} correctly"
...
class OutputContract:
    ...
    def verify(self, cell_outputs: Dict[str, str]) -> Tuple[bool, str]:
        ...
        return True, f"✓ Cell '{self.cell}' output matches contract"
...
class SemanticAnchor:
    ...
    def verify_exact(self, output: str) -> Tuple[bool, str]:
        ...
        if self.anchor.lower() in output.lower():
            return True, f"✓ Anchor '{self.anchor}' found in {self.cell}"
        if self.required:
            return False, f"✗ Required anchor '{self.anchor}' missing from {self.cell}"
        return True, f"⚠ Optional anchor '{self.anchor}' not found in {self.cell}"
    ...
    def verify_semantic(self, output: str, embedder) -> Tuple[bool, float, str]:
        ...
        if not sentences:
            ...
            return True, 0.0, f"⚠ No content for optional anchor '{self.anchor}'"
        ...
        if max_sim >= self.similarity_threshold:
            return True, max_sim, f"✓ Anchor '{self.anchor}' found (sim={max_sim:.2f})"

        if self.required:
            return False, max_sim, f"✗ Anchor '{self.anchor}' not found (best sim={max_sim:.2f} < {self.similarity_threshold})"
        return True, max_sim, f"⚠ Optional anchor '{self.anchor}' weak match (sim={max_sim:.2f})"
...
class SnapshotCapture:
    ...
    def freeze(
        ...
        print(f"  • Deterministic replay: rvbbit test replay {snapshot_name} --mode deterministic")
        print(f"  • Contract verification: rvbbit test replay {snapshot_name} --mode contracts")
        print(f"  • Full comparison:      rvbbit test replay {snapshot_name} --mode full")
...
class SnapshotValidator:
    ...
    def _validate_contracts(self, snapshot: Dict, result: ReplayResult, verbose: bool):
        ...
        if contracts.cell_sequence:
            if routing_path == contracts.cell_sequence:
                result.contract_results.append((True, f"✓ Cell sequence matches: {' → '.join(routing_path)}"))
            else:
    ...
    def _validate_deterministic(self, snapshot: Dict, result: ReplayResult, verbose: bool):
        ...
        try:
            ...
            result.checks.append(f"✓ Mock LLM created with {len(mock_llm.responses)} frozen responses")
...
"""
    ...
    def judge(self, expected: str, actual: str) -> Dict[str, Any]:
        ...
        """
        from rvbbit.agent import LLMWrapper
