...
Usage:
    from rvbbit.toon_transport import transform_messages_for_transport
...
def is_toon_transport_enabled() -> bool:
    ...
    import os
    setting = os.environ.get("RVBBIT_TOON_TRANSPORT", "1").lower()
...
def get_transport_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "toon_available": TOON_AVAILABLE,
        "min_rows": get_min_rows_threshold() if TOON_AVAILABLE else 5,
        "env_var": os.environ.get("RVBBIT_TOON_TRANSPORT", "(not set)")
    }
