...
def cascade_write(
    ...
    Args:
        ...
        cell: Cell definition dict. For hitl cells: {"name": "...", "hitl": "<html>...", "handoffs": [...]}
              ...
              For LLM cells: {"name": "...", "instructions": "...", "traits": [...]}
    ...
    try:
        ...
        if action == "create":
            if cascade_path.exists():
                ...
                console.print(f"[yellow]⚠ Cascade '{cascade_id}' already exists - updating metadata only[/yellow]")
        ...
        console.print(f"[green]✓ Cascade '{cascade_id}' updated ({action})[/green]")
...
def cascade_read(cascade_id: str, session_id: str | None = None) -> Dict[str, Any]:
    ...
    # 3. Traits directory
    search_paths.append(Path(config.root_dir) / "traits" / f"{cascade_id}.yaml")
