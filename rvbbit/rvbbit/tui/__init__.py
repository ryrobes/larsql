"""
RVBBIT TUI - Alice-powered Terminal Dashboard for Cascade Monitoring.

This module provides an interactive TUI for monitoring cascade sessions,
powered by the Alice Terminal framework.

Usage:
    rvbbit tui                              # Launch session browser
    rvbbit tui --cascade path/to.yaml       # Monitor specific cascade
"""
...
def find_alice_executable() -> Optional[str]:
    ...
    # Check common locations relative to rvbbit
    rvbbit_root = Path(__file__).resolve().parent.parent.parent.parent
    possible_paths = [
        rvbbit_root / 'alice' / 'venv' / 'bin' / 'alice',
        rvbbit_root.parent / 'alice' / 'venv' / 'bin' / 'alice',
...
def launch_tui(
    ...
    if cascade:
        ...
        from rvbbit.alice_generator import generate_alice_yaml, load_cascade
        ...
        print("Launching RVBBIT Session Browser...")
