...
from rvbbit.browser_manager import (
    BrowserSession,
...
class TestCellWithBrowserConfig:
    ...
    def test_cascade_with_browser_cell(self):
        ...
        assert cascade.cells[0].browser is not None
        assert cascade.cells[0].browser.stability_detection is True
...
class TestBrowserCascadeLoading:
    ...
    def test_load_browser_demo(self, tmp_path):
        ...
        assert cascade.cells[0].browser is not None
        assert cascade.cells[0].browser.url == "{{ input.url }}"
        assert cascade.cells[0].browser.stability_detection is True
        assert cascade.cells[0].browser.stability_wait == 2.0
