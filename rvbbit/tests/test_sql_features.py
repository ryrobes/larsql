"""
Tests for RVBBIT SQL enhancements.
...
from rvbbit.sql_tools.udf import (
    _parse_duration,
    _cache_get,
    _cache_set,
    _udf_cache,
...
class TestEXPLAIN:
    """Test EXPLAIN RVBBIT MAP functionality."""

    def test_explain_detection(self):
        """Should detect EXPLAIN prefix."""
        query = "EXPLAIN RVBBIT MAP 'cascade.yaml' USING (SELECT * FROM t)"

        # Without conn, should return error
        result = rewrite_rvbbit_syntax(query, duckdb_conn=None)
    ...
    def test_explain_only_for_map(self):
        ...
        query = "EXPLAIN RVBBIT RUN 'cascade.yaml' USING (SELECT * FROM t)"

        with pytest.raises(RVBBITSyntaxError, match="only supported for RVBBIT MAP"):
            rewrite_rvbbit_syntax(query, duckdb_conn=duckdb.connect())
...
class TestMAPDISTINCT:
    ...
    def test_parse_distinct_keyword(self):
        ...
        query = "RVBBIT MAP DISTINCT 'cascade.yaml' USING (SELECT * FROM t)"
        stmt = _parse_rvbbit_statement(query)
    ...
    def test_distinct_rewrite(self):
        ...
        query = "RVBBIT MAP DISTINCT 'test.yaml' USING (SELECT name FROM t)"
        rewritten = rewrite_rvbbit_syntax(query)
...
class TestTableMaterialization:
    ...
    def test_materialization_in_rewrite(self):
        ...
        assert 'rvbbit_materialize_table' in rewritten
...
class TestCombinedFeatures:
    ...
    def test_explain_with_schema(self):
        ...
        query = """
        EXPLAIN RVBBIT MAP 'cascade.yaml' AS (
            brand VARCHAR,
            score DOUBLE
        )
...
class TestBackwardCompatibility:
    ...
    def test_map_with_alias_still_works(self):
        ...
        query = "RVBBIT MAP 'cascade.yaml' AS brand USING (SELECT * FROM t)"
        stmt = _parse_rvbbit_statement(query)
    ...
    def test_map_run_unchanged(self):
        """RVBBIT RUN should work unchanged."""
        query = """
