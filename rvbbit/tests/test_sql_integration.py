"""
Integration tests for RVBBIT SQL features.
...
import os
import tempfile
from rvbbit.sql_tools.udf import register_rvbbit_udf, clear_udf_cache
from rvbbit.sql_rewriter import rewrite_rvbbit_syntax


@pytest.fixture
def db_conn():
    """Create DuckDB connection with RVBBIT UDFs registered."""
...
class TestSchemaAwareIntegration:
    ...
    def test_basic_rvbbit_udf(self, db_conn):
        """Test basic rvbbit_udf function."""
        result = db_conn.execute("""
            SELECT rvbbit_udf('Say hello', 'world') as greeting
        """).fetchone()
    ...
    def test_cache_hit(self, db_conn):
        ...
        result1 = db_conn.execute("""
            SELECT rvbbit_udf('Extract brand', 'Apple iPhone') as brand
        """).fetchone()
...
class TestQueryNormalization:
    ...
    def test_query_with_comments(self, db_conn):
        ...
        query = """
        -- This is a comment
        RVBBIT MAP 'cascade.yaml' -- inline comment
        USING (
...
class TestRealWorldScenarios:
    ...
    def test_product_enrichment_workflow(self, db_conn):
        ...
        """
        explain_result = rewrite_rvbbit_syntax(explain_query, duckdb_conn=db_conn)
        ...
        WITH (dedupe_by='product_name', cache='1d')
        """
        stmt = _parse_rvbbit_statement(actual_query)
