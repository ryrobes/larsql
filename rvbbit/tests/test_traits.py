"""
Tests for Traits (Tool) Registry.

The traits module manages tool registration and retrieval.
These tests verify:
1. Tool registration
...
@pytest.fixture
def fresh_registry():
    """Create a fresh TraitRegistry for isolated testing."""
    from rvbbit.trait_registry import TraitRegistry
...
class TestTraitRegistry:
    """Test TraitRegistry class."""

    def test_registry_init(self, fresh_registry):
        """Registry initializes empty."""
        assert fresh_registry.get_all_traits() == {}

    def test_register_single_tool(self, fresh_registry, sample_tools):
        """Register a single tool."""
        fresh_registry.register_trait("add", sample_tools["add"])

        assert fresh_registry.get_trait("add") is sample_tools["add"]

    def test_register_multiple_tools(self, fresh_registry, sample_tools):
        """Register multiple tools."""
        fresh_registry.register_trait("add", sample_tools["add"])
        fresh_registry.register_trait("greet", sample_tools["greet"])
        fresh_registry.register_trait("process", sample_tools["process_data"])

        all_tools = fresh_registry.get_all_traits()
    ...
    def test_get_nonexistent_tool(self, fresh_registry):
        ...
        result = fresh_registry.get_trait("nonexistent")
    ...
    def test_overwrite_tool(self, fresh_registry):
        ...
        fresh_registry.register_trait("tool", original)
        assert fresh_registry.get_trait("tool")() == "original"

        fresh_registry.register_trait("tool", replacement)
        assert fresh_registry.get_trait("tool")() == "replacement"
    ...
    def test_tools_are_callable(self, fresh_registry, sample_tools):
        ...
        add_fn = fresh_registry.get_trait("add")
        assert add_fn(2, 3) == 5

        greet_fn = fresh_registry.get_trait("greet")
...
class TestModuleFunctions:
    ...
    def test_register_and_get_trait(self):
        """register_trait and get_trait work with global registry."""
        from rvbbit.trait_registry import register_trait, get_trait
        ...
        # Register
        register_trait("test_double", my_tool)

        # Retrieve
        retrieved = get_trait("test_double")
        assert retrieved is my_tool
        assert retrieved(5) == 10

    def test_get_registry(self):
        """get_registry returns the global registry."""
        from rvbbit.trait_registry import get_registry, TraitRegistry

        registry = get_registry()
        assert isinstance(registry, TraitRegistry)
...
class TestToolMetadata:
    ...
    def test_docstring_preserved(self, fresh_registry):
        ...
        fresh_registry.register_trait("documented", documented_tool)
        tool = fresh_registry.get_trait("documented")
    ...
    def test_name_preserved(self, fresh_registry):
        ...
        fresh_registry.register_trait("named", named_tool)
        tool = fresh_registry.get_trait("named")
    ...
    def test_annotations_preserved(self, fresh_registry):
        ...
        fresh_registry.register_trait("typed", typed_tool)
        tool = fresh_registry.get_trait("typed")
...
class TestToolTypes:
    ...
    def test_lambda_function(self, fresh_registry):
        ...
        fresh_registry.register_trait("square", lambda x: x * x)

        tool = fresh_registry.get_trait("square")
        assert tool(4) == 16
    ...
    def test_class_method(self, fresh_registry):
        ...
        calc = Calculator()
        fresh_registry.register_trait("multiply", calc.multiply)

        tool = fresh_registry.get_trait("multiply")
        assert tool(3, 4) == 12
    ...
    def test_callable_class(self, fresh_registry):
        ...
        adder = Adder(10)
        fresh_registry.register_trait("add_10", adder)

        tool = fresh_registry.get_trait("add_10")
        assert tool(5) == 15
    ...
    def test_async_function(self, fresh_registry):
        ...
        fresh_registry.register_trait("async_double", async_tool)

        tool = fresh_registry.get_trait("async_double")
...
class TestEdgeCases:
    ...
    def test_tool_with_defaults(self, fresh_registry):
        ...
        fresh_registry.register_trait("defaults", with_defaults)

        tool = fresh_registry.get_trait("defaults")
    ...
    def test_tool_with_args_kwargs(self, fresh_registry):
        ...
        fresh_registry.register_trait("flexible", flexible)

        tool = fresh_registry.get_trait("flexible")
    ...
    def test_tool_returning_none(self, fresh_registry):
        ...
        fresh_registry.register_trait("void", void_tool)

        tool = fresh_registry.get_trait("void")
        assert tool() is None
    ...
    def test_tool_with_special_name(self, fresh_registry):
        ...
        fresh_registry.register_trait("my_tool_v2_beta", tool_v2_beta)

        tool = fresh_registry.get_trait("my_tool_v2_beta")
        assert tool() == "v2"
    ...
    def test_empty_name(self, fresh_registry):
        ...
        fresh_registry.register_trait("", empty_named)

        tool = fresh_registry.get_trait("")
        assert tool() == "empty"
...
class TestGetAllTackle:
    """Test get_all_traits functionality."""

    def test_get_all_returns_dict(self, fresh_registry):
        """get_all_traits returns a dictionary."""
        result = fresh_registry.get_all_traits()
        assert isinstance(result, dict)

    def test_get_all_after_registration(self, fresh_registry, sample_tools):
        """get_all_traits returns all registered tools."""
        fresh_registry.register_trait("add", sample_tools["add"])
        fresh_registry.register_trait("greet", sample_tools["greet"])
    ...
    def test_get_all_returns_reference(self, fresh_registry, sample_tools):
        ...
        assert fresh_registry.get_trait("new_tool") is not None
...
class TestToolSchemaIntegration:
    ...
    def test_registered_tool_schema(self, fresh_registry):
        ...
        fresh_registry.register_trait("analyze", analyze)
        tool = fresh_registry.get_trait("analyze")
