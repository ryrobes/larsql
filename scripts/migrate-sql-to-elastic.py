...
from lars.elastic import get_elastic_client, create_sql_schema_index, bulk_index_sql_schemas
...
def main():
    ...
    # Ensure index exists
    if not es.indices.exists(index='lars_sql_schemas'):
    ...
    try:
        ...
        if indexed_count < len(documents):
            ...
            all_docs = es.search(index='lars_sql_schemas', body={'size': 200, '_source': False})
    ...
    # Refresh and verify
    es.indices.refresh(index='lars_sql_schemas')
    count = es.count(index='lars_sql_schemas')
    print()
