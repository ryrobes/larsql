...
def migrate_snapshot(snapshot_file: Path):
    """Migrate a snapshot JSON file to use cells instead of phases"""
    ...
    if "execution" in data:
        # Rename phases → cells
        if "phases" in data["execution"]:
            data["execution"]["cells"] = data["execution"].pop("phases")

        # Update each cell (formerly phase)
        if "cells" in data["execution"]:
            for cell in data["execution"]["cells"]:
                # Update field names within each cell
                if "phase_name" in cell:
                    cell["cell_name"] = cell.pop("phase_name")
                if "sounding_index" in cell:
                    cell["candidate_index"] = cell.pop("sounding_index")

    # Update expectations
    if "expectations" in data:
        if "phases_executed" in data["expectations"]:
            data["expectations"]["cells_executed"] = data["expectations"].pop("phases_executed")
