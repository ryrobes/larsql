...
function ArtifactsView({
  ...
  const fetchArtifacts = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5050/api/artifacts?${params}`);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5050/api/artifacts/export-pdf', {
        method: 'POST',
  ...
  const handleExportHTML = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5050/api/artifacts/export-html', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          artifact_ids: Array.from(selectedArtifacts)
...
function ArtifactCard({ artifact, onClick, isSelected, onToggleSelect }) {
  ...
  const thumbnailUrl = `http://localhost:5050/api/images/artifacts/${artifact.id}.png`;
