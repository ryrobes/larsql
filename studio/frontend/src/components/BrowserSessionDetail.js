...
function BrowserSessionDetail({ sessionPath, onBack }) {
  ...
  const fetchSession = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5050/api/browser-sessions/${sessionPath}`);
                ...
                {session.screenshots?.length > 0 ? (
                  <>
                    ...
                    <div className="screenshot-preview">
                      {selectedScreenshot ? (
                        <img
                          src={`http://localhost:5050/api/browser-media/${session.path}/screenshots/${selectedScreenshot.filename || selectedScreenshot}`}
                    ...
                    <div className="screenshot-strip">
                      {session.screenshots.map((screenshot, index) => {
                        ...
                        return (
                          ...
                          >
                            <img
                              src={`http://localhost:5050/api/browser-media/${session.path}/screenshots/${thumbFilename}`}
            ...
            {activeTab === 'video' && (
              <div className="video-panel">
                {session.video ? (
                  <video
                    controls
                    src={`http://localhost:5050/api/browser-media/${session.path}/video/${session.video}`}
                    className="video-player"
                  />
                ) : session.has_video ? (
                  <video
                    controls
                    src={`http://localhost:5050/api/browser-media/${session.path}/video/session.webm`}
                    className="video-player"
                  />
                ) : (
                  <div className="empty-tab">
                ...
                {session.dom_snapshots?.length > 0 ? (
                  <>
                    ...
                    <div className="dom-preview">
                      {selectedDom ? (
                        <DomSnapshotViewer
                          url={`http://localhost:5050/api/browser-media/${session.path}/dom_snapshots/${selectedDom}`}
                        />
                      ) : (
                        <div className="no-selection">
