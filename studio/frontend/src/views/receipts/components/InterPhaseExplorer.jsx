...
import './InterPhaseExplorer.css';

const InterPhaseExplorer = ({ sessionId }) => {
  ...
  if (loading) {
    return (
      <div className="inter-phase-explorer loading">
  ...
  if (error) {
    return (
      <div className="inter-phase-explorer error">
  ...
  if (!data?.cells?.length) {
    return (
      <div className="inter-phase-explorer empty">
        <Icon icon="mdi:swap-horizontal" width={32} />
        <span>No inter-phase context data available</span>
      </div>
    );
  }

  return (
    <div className="inter-phase-explorer">
...
export default InterPhaseExplorer;
