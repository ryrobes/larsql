...
import './IntraPhaseExplorer.css';

const IntraPhaseExplorer = ({ sessionId }) => {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:5050/api/context-assessment/intra-phase/${sessionId}`
        );
        if (!res.ok) throw new Error('Failed to fetch intra-phase data');
  ...
  if (loading) {
    return (
      <div className="intra-phase-explorer loading">
        <Icon icon="mdi:loading" className="spin" width={24} />
        <span>Loading intra-phase data...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="intra-phase-explorer error">
  ...
  if (!data?.cells?.length) {
    return (
      <div className="intra-phase-explorer empty">
        <Icon icon="mdi:rotate-right" width={32} />
        <span>No intra-phase compression data available</span>
      </div>
    );
  }

  return (
    <div className="intra-phase-explorer">
...
export default IntraPhaseExplorer;
