...
const getInitialCascadeFilter = () => {
  try {
    return localStorage.getItem(STORAGE_KEY_CASCADE_FILTER) || 'all';
  } catch (e) {
    return 'all';
  }
};
...
const getInitialCellFilter = () => {
  try {
    return localStorage.getItem(STORAGE_KEY_CELL_FILTER) || 'all';
  } catch (e) {
    return 'all';
  }
};
...
const TrainingView = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      // Extract unique cascades and cells for filters
      const cascades = [...new Set(stats_data.map(s => s.cascade_id))].sort();
      const cells = [...new Set(stats_data.map(s => s.cell_name))].sort();
      setCascadeOptions(cascades);
  ...
  const fetchExamples = async () => {
    try {
      ...
      if (cascadeFilter && cascadeFilter !== 'all') {
        params.append('cascade_id', cascadeFilter);
      }
      if (cellFilter && cellFilter !== 'all') {
        params.append('cell_name', cellFilter);
      }
  ...
  // Handle filter changes (persist to localStorage)
  const handleCascadeFilterChange = useCallback((value) => {
    setCascadeFilter(value);
    try {
      localStorage.setItem(STORAGE_KEY_CASCADE_FILTER, value);
    } catch (e) {}
  }, []);
  ...
  const handleCellFilterChange = useCallback((value) => {
    setCellFilter(value);
    try {
      localStorage.setItem(STORAGE_KEY_CELL_FILTER, value);
    } catch (e) {}
  }, []);
  ...
  useEffect(() => {
    fetchExamples();
  }, [cascadeFilter, cellFilter, showTrainableOnly]);
    ...
    <div className="training-view">
      ...
      <div className="training-filters">
        <div className="training-filter-group">
          ...
          <select
            value={cascadeFilter}
            onChange={(e) => handleCascadeFilterChange(e.target.value)}
            className="training-filter-select"
          >
            <option value="all">All Cascades</option>
            {cascadeOptions.map(cascade => (
              <option key={cascade} value={cascade}>{cascade}</option>
            ))}
          </select>
        </div>
