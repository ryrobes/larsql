...
function App() {
  ...
  // ============================================
  // TEMPORARY: Force new AppShell system until old_frontend split is complete
  // TODO: Remove this when old system is moved to old_frontend/

  // const newShellRoutes = ['#/studio', '#/console', '#/cascades', '#/outputs', '#/receipts', '#/interrupts', '#/explore', '#/evolution'];

  // FORCE NEW SYSTEM ONLY - old routes will show "not found" until migrated
  const [useNewShell] = useState(true);

  // OLD DUAL-MODE LOGIC (commented out):
  ...
  const handleFreezeSubmit = async (sessionId, snapshotName, description) => {
    try {
      const response = await fetch('http://localhost:5050/api/test/freeze', {
        method: 'POST',
        headers: {
    ...
    const handleHashChange = async () => {
      ...
      if (route.view === 'cascades') {
        ...
        if (selectedCascadeId !== route.cascadeId) {
          try {
            const response = await fetch('http://localhost:5050/api/cascade-definitions');
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource('http://localhost:5050/api/events/stream');
      ...
      try {
        ...
        switch (event.type) {
          ...
          case 'checkpoint_waiting':
            ...
            fetch('http://localhost:5050/api/sessions/blocked?exclude_research_cockpit=true')
              .then(res => res.json())
  ...
  const fetchBlockedCount = async () => {
    try {
      const response = await fetch('http://localhost:5050/api/sessions/blocked?exclude_research_cockpit=true');
      const data = await response.json();
