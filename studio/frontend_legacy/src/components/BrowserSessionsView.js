...
function BrowserSessionsView({
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:5050/api/browser-sessions');
  ...
  const handleDelete = async (e, session) => {
    ...
    try {
      const response = await fetch(`http://localhost:5050/api/browser-sessions/${session.path}`, {
        method: 'DELETE'
      });
      const data = await response.json();
            ...
            {sortedSessions.map(session => (
              ...
              >
                ...
                <div className="session-thumbnail">
                  {session.thumbnail ? (
                    <img
                      src={`http://localhost:5050/api/browser-media/${session.thumbnail}`}
