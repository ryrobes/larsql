...
function FlowRegistryView({
  ...
  const fetchFlows = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:5050/api/browser-flows');
  ...
  const fetchFlowDetails = async (flowId) => {
    try {
      const response = await fetch(`http://localhost:5050/api/browser-flows/${flowId}`);
      const data = await response.json();
  ...
  const handleRegisterAsTool = async (flowId, e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5050/api/browser-flows/${flowId}/register`, {
        method: 'POST'
      });
      const data = await response.json();
      ...
      {showParameterEditor && selectedFlowData && (
        <ParameterEditorModal
          ...
          onSave={async (updatedFlow) => {
            try {
              const response = await fetch(`http://localhost:5050/api/browser-flows/${updatedFlow.flow_id}`, {
                method: 'PUT',
