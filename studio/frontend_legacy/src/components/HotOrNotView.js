...
function HotOrNotView({ onBack }) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const response = await fetch('http://localhost:5050/api/hotornot/stats');
  ...
  const fetchQueue = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5050/api/hotornot/queue?limit=50&show_all=${showAllSoundings}`);
      const data = await response.json();
      setQueue(data);
      setCurrentIndex(0);
  ...
  const handleRate = async (isGood) => {
    ...
    try {
      await fetch('http://localhost:5050/api/hotornot/rate', {
        method: 'POST',
  ...
  const handlePrefer = async (preferredIndex) => {
    ...
    try {
      await fetch('http://localhost:5050/api/hotornot/prefer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: current.session_id,
  ...
  const handleFlag = async () => {
    ...
    try {
      await fetch('http://localhost:5050/api/hotornot/flag', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: current.session_id,
