...
function ResearchCockpit({
  ...
  const fetchSavedSession = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5050/api/research-sessions?limit=100`);
      ...
      if (!data.error && data.sessions) {
        ...
        if (saved) {
          ...
          // Fetch full details
          const detailRes = await fetch(`http://localhost:5050/api/research-sessions/${saved.id}`);
  ...
  const fetchSessionData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5050/api/session/${sessionId}`);
      const data = await res.json();
  ...
  const fetchCheckpoint = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5050/api/checkpoints?session_id=${sessionId}&include_all=true`);
      const data = await res.json();
  ...
  const handleCreateBranch = async (checkpointIndex, newResponse) => {
    ...
    try {
      ...
      if (!researchSessionId) {
        ...
        const checkRes = await fetch(`http://localhost:5050/api/research-sessions?limit=100`);
      ...
      const res = await fetch('http://localhost:5050/api/research-sessions/branch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
