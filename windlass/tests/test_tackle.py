"""
Tests for Tackle (Tool) Registry.

The tackle module manages tool registration and retrieval.
These tests verify:
1. Tool registration
...
@pytest.fixture
def fresh_registry():
    """Create a fresh ToolRegistry for isolated testing."""
    from windlass.tackle import ToolRegistry
    return ToolRegistry()
...
class TestToolRegistry:
    ...
    def test_registry_init(self, fresh_registry):
        ...
        assert fresh_registry.get_all_tackle() == {}

    def test_register_single_tool(self, fresh_registry, sample_tools):
        """Register a single tool."""
        fresh_registry.register_tackle("add", sample_tools["add"])

        assert fresh_registry.get_tackle("add") is sample_tools["add"]

    def test_register_multiple_tools(self, fresh_registry, sample_tools):
        """Register multiple tools."""
        fresh_registry.register_tackle("add", sample_tools["add"])
        fresh_registry.register_tackle("greet", sample_tools["greet"])
        fresh_registry.register_tackle("process", sample_tools["process_data"])

        all_tools = fresh_registry.get_all_tackle()
    ...
    def test_get_nonexistent_tool(self, fresh_registry):
        ...
        result = fresh_registry.get_tackle("nonexistent")
    ...
    def test_overwrite_tool(self, fresh_registry):
        ...
        fresh_registry.register_tackle("tool", original)
        assert fresh_registry.get_tackle("tool")() == "original"

        fresh_registry.register_tackle("tool", replacement)
        assert fresh_registry.get_tackle("tool")() == "replacement"
    ...
    def test_tools_are_callable(self, fresh_registry, sample_tools):
        ...
        add_fn = fresh_registry.get_tackle("add")
        assert add_fn(2, 3) == 5

        greet_fn = fresh_registry.get_tackle("greet")
...
class TestModuleFunctions:
    ...
    def test_register_and_get_tackle(self):
        """register_tackle and get_tackle work with global registry."""
        from windlass.tackle import register_tackle, get_tackle
        ...
        # Register
        register_tackle("test_double", my_tool)

        # Retrieve
        retrieved = get_tackle("test_double")
        assert retrieved is my_tool
        assert retrieved(5) == 10

    def test_get_registry(self):
        """get_registry returns the global registry."""
        from windlass.tackle import get_registry, ToolRegistry
...
class TestToolMetadata:
    ...
    def test_docstring_preserved(self, fresh_registry):
        ...
        fresh_registry.register_tackle("documented", documented_tool)
        tool = fresh_registry.get_tackle("documented")
    ...
    def test_name_preserved(self, fresh_registry):
        ...
        fresh_registry.register_tackle("named", named_tool)
        tool = fresh_registry.get_tackle("named")
    ...
    def test_annotations_preserved(self, fresh_registry):
        ...
        fresh_registry.register_tackle("typed", typed_tool)
        tool = fresh_registry.get_tackle("typed")
...
class TestToolTypes:
    ...
    def test_lambda_function(self, fresh_registry):
        ...
        fresh_registry.register_tackle("square", lambda x: x * x)

        tool = fresh_registry.get_tackle("square")
        assert tool(4) == 16
    ...
    def test_class_method(self, fresh_registry):
        ...
        calc = Calculator()
        fresh_registry.register_tackle("multiply", calc.multiply)

        tool = fresh_registry.get_tackle("multiply")
        assert tool(3, 4) == 12
    ...
    def test_callable_class(self, fresh_registry):
        ...
        adder = Adder(10)
        fresh_registry.register_tackle("add_10", adder)

        tool = fresh_registry.get_tackle("add_10")
        assert tool(5) == 15
    ...
    def test_async_function(self, fresh_registry):
        ...
        fresh_registry.register_tackle("async_double", async_tool)

        tool = fresh_registry.get_tackle("async_double")
...
class TestEdgeCases:
    ...
    def test_tool_with_defaults(self, fresh_registry):
        ...
        fresh_registry.register_tackle("defaults", with_defaults)

        tool = fresh_registry.get_tackle("defaults")
    ...
    def test_tool_with_args_kwargs(self, fresh_registry):
        ...
        fresh_registry.register_tackle("flexible", flexible)

        tool = fresh_registry.get_tackle("flexible")
    ...
    def test_tool_returning_none(self, fresh_registry):
        ...
        fresh_registry.register_tackle("void", void_tool)

        tool = fresh_registry.get_tackle("void")
        assert tool() is None
    ...
    def test_tool_with_special_name(self, fresh_registry):
        ...
        fresh_registry.register_tackle("my_tool_v2_beta", tool_v2_beta)

        tool = fresh_registry.get_tackle("my_tool_v2_beta")
        assert tool() == "v2"
    ...
    def test_empty_name(self, fresh_registry):
        ...
        fresh_registry.register_tackle("", empty_named)

        tool = fresh_registry.get_tackle("")
        assert tool() == "empty"
...
class TestGetAllTackle:
    """Test get_all_tackle functionality."""

    def test_get_all_returns_dict(self, fresh_registry):
        """get_all_tackle returns a dictionary."""
        result = fresh_registry.get_all_tackle()
        assert isinstance(result, dict)

    def test_get_all_after_registration(self, fresh_registry, sample_tools):
        """get_all_tackle returns all registered tools."""
        fresh_registry.register_tackle("add", sample_tools["add"])
        fresh_registry.register_tackle("greet", sample_tools["greet"])
    ...
    def test_get_all_returns_reference(self, fresh_registry, sample_tools):
        ...
        assert fresh_registry.get_tackle("new_tool") is not None
...
class TestToolSchemaIntegration:
    ...
    def test_registered_tool_schema(self, fresh_registry):
        ...
        from windlass.utils import get_tool_schema
        ...
        fresh_registry.register_tackle("analyze", analyze)
        tool = fresh_registry.get_tackle("analyze")
