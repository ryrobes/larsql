"""
Auto-Context System for Windlass
...
class InterPhaseContextBuilder:
    ...
    def build_phase_context(
        ...
        anchors_config = self._get_anchors_config(current_phase)
        selection_config = self._get_selection_config(current_phase)
        ...
        selected_hashes = self._select_messages(
            ...
            current_task=current_phase.instructions or "",
    ...
    def _get_anchors_config(self, phase: 'PhaseConfig') -> 'AnchorConfig':
    ...
    def _get_selection_config(self, phase: 'PhaseConfig') -> 'SelectionConfig':
    ...
    def _get_phase_output(self, phase_name: str) -> Optional[str]:
        ...
        output_key = f"output_{phase_name}"
    ...
    def _get_callout_messages(self, executed_phases: List[str]) -> List[Dict]:
        ...
        for entry in self.echo.history:
            ...
            if meta.get("is_callout") and meta.get("phase_name") in executed_phases:
                callouts.append({
    ...
    def _get_error_messages(self, executed_phases: List[str]) -> List[Dict]:
        ...
        for entry in self.echo.history:
            ...
            if (
                meta.get("phase_name") in executed_phases and
                any(kw in content for kw in ["error", "exception", "failed", "traceback"])
            ):
                errors.append({
                    "role": entry.get("role", "user"),
                    "content": f"[Error from {meta.get('phase_name', 'unknown')}]\n{entry.get('content', '')}",
                    "_anchor": True,
                    "_anchor_type": "error",
                    "_hash": entry.get("_hash")
                })
    ...
    def _get_candidate_cards(
        ...
        candidates = []
        for card in cards:
            if card.get("phase_name") in executed_phases:
