"""
Windlass SQL Client - Query Windlass DuckDB server from Python.

This provides a pandas-like interface for executing SQL queries
with windlass_udf() and windlass_cascade_udf() from remote clients.
...
class WindlassClient:
    ...
    def __init__(self, base_url: str = 'http://localhost:5001', session_id: Optional[str] = None):
        """
        Initialize Windlass SQL client.

        Args:
            base_url: Windlass server URL (default: http://localhost:5001)
    ...
    def execute(self, query: str, session_id: Optional[str] = None, format: str = 'records') -> pd.DataFrame:
        ...
        Args:
            query: SQL query to execute (can include windlass_udf, windlass_cascade_udf)
    ...
    def health_check(self) -> Dict[str, Any]:
        """
        Check if Windlass server is healthy and UDFs are registered.
...
def execute_sql(query: str, base_url: str = 'http://localhost:5001') -> pd.DataFrame:
    ...
    Example:
        ...
        df = execute_sql("SELECT windlass_udf('Extract brand', 'Apple iPhone') as brand")
        print(df)
    """
    client = WindlassClient(base_url)
