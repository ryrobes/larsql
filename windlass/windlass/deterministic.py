"""
Deterministic phase execution for Windlass.
...
from .cascade import PhaseConfig, RetryConfig
...
class DeterministicExecutionError(Exception):
    ...
    def __init__(self, message: str, phase_name: str, tool: str, inputs: Dict = None, original_error: Exception = None):
    ...
    if tool_type == "registered":
        ...
        def shell_executor(**kwargs) -> Dict[str, Any]:
            ...
            for key, value in kwargs.items():
                env[f"WINDLASS_{key.upper()}"] = str(value)
...
def execute_deterministic_phase(
    ...
    Returns:
        Tuple of (result, next_phase_name)
    ...
    if phase.tool in ("sql_data", "python_data", "js_data", "clojure_data", "windlass_data"):
        ...
        # Enable materialization by default for sql_data (creates _phase_name temp tables)
