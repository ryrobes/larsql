...
def example_soundings_integration():
    ...
    for i in range(factor):
        # Run sounding attempt with timing
        with TimingContext() as timer:
            output = self._run_phase_logic(...)
        ...
        log_echo(
            ...
            role="sounding",
            depth=depth,
            sounding_index=i,  # Track which attempt
            is_winner=False,   # Updated later when winner selected
            phase_name=phase.name,
            cascade_id=cascade_config.cascade_id,
            duration_ms=timer.get_duration_ms(),
            content=output,  # Full sounding output
            metadata={
                "attempt": i,
                "factor": factor,
...
def test_echo_logging():
    ...
    from windlass.echoes import query_echoes_jsonl, query_echoes_parquet
    ...
    for entry in entries:
        print(f"  - {entry['node_type']}: {entry.get('phase_name', 'N/A')}")
