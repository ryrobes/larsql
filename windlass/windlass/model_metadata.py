"""
Model Metadata Cache for Windlass - Context Limit Filtering

Fetches model metadata from OpenRouter API and caches it locally.
Used to filter models for multi-model soundings based on context limits.
...
class ModelMetadataCache:
    ...
    during multi-model soundings execution.
    """
    ...
    def __init__(
        ...
        Args:
            cache_path: Path to cache file (default: $WINDLASS_ROOT/data/.model_cache.json)
    ...
    async def filter_viable_models(
        ...
        for model in models:
            ...
            if limit is None:
                ...
                logger.info(
                    f"Filtered {model} from sounding: "
