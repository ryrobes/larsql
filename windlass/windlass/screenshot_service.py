...
class ScreenshotService:
    ...
    def capture_htmx_render(
        ...
        """
        Capture screenshot of HTMX render (overwrites previous for this phase/sounding).

        Path strategy (overwrites on each call):
        - No sounding: /images/{session}/{phase}/htmx_latest.png
        - With sounding: /images/{session}/{phase}/htmx_s{N}.png

        This keeps only the FINAL state, iterations are overwritten.
        ...
        # Build path that overwrites on each render
        if sounding_index is not None:
            filename = f"{render_type}_s{sounding_index}.png"
        else:
