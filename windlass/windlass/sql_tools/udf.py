"""
windlass_udf() - LLM-powered SQL user-defined function.
...
def windlass_udf_impl(
    ...
    """
    Core implementation of windlass_udf.
    ...
    Args:
        ...
        model: Optional model override (default: uses WINDLASS_DEFAULT_MODEL)
    ...
    try:
        ...
        import logging
        import traceback
        logging.getLogger(__name__).error(f"windlass_udf error for '{instructions}': {e}\n{traceback.format_exc()}")
...
def windlass_cascade_udf_impl(
    ...
    try:
        ...
        for phase_item in result.get("lineage", []):
            ...
            if phase_name:
                outputs[phase_name] = phase_output
        ...
        import logging
        import traceback
        logging.getLogger(__name__).error(f"windlass_cascade_udf error for '{cascade_path}': {e}\n{traceback.format_exc()}")
        return json.dumps({"error": str(e), "status": "failed"})


def register_windlass_udf(connection: duckdb.DuckDBPyConnection, config: Dict[str, Any] = None):
    """
    Register windlass_udf as a DuckDB user-defined function.
    ...
    Example:
        conn = duckdb.connect()
        register_windlass_udf(conn, {"model": "anthropic/claude-haiku-4.5"})
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).warning(f"Could not register windlass_udf: {e}")
    ...
    try:
        def cascade_udf_wrapper(cascade_path: str, inputs_json: str) -> str:
            ...
            return windlass_cascade_udf_impl(cascade_path, inputs_json)
        ...
        import logging
        logging.getLogger(__name__).warning(f"Could not register windlass_cascade_udf: {e}")
