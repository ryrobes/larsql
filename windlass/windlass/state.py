...
class PhaseProgress:
    ...
    def __init__(self, phase_name: str):
        ...
        # Sounding tracking
        self.sounding_index: Optional[int] = None
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "sounding": {
                "index": self.sounding_index,
                "factor": self.sounding_factor,
                "stage": self.sounding_stage
            } if self.sounding_index is not None else None,
            "reforge": {
...
class StateManager:
    ...
    def get_phase_progress(self, session_id: str, phase_name: str) -> PhaseProgress:
        """Get or create phase progress tracker."""
        key = f"{session_id}:{phase_name}"
        if key not in self._phase_progress:
            self._phase_progress[key] = PhaseProgress(phase_name)
        return self._phase_progress[key]

    def clear_phase_progress(self, session_id: str, phase_name: str):
...
def get_phase_progress(session_id: str, phase_name: str) -> PhaseProgress:
    """Get or create detailed phase progress tracker."""
    return _state_manager.get_phase_progress(session_id, phase_name)
...
def update_phase_progress(
    ...
    *,
    ...
    within a phase (which turn, which attempt, which sounding, etc.)
    """
    progress = _state_manager.get_phase_progress(session_id, phase_name)
    ...
    if sounding_index is not None:
        progress.sounding_index = sounding_index
    ...
    _state_manager.update(session_id, cascade_id, "running", phase_name, depth)


def clear_phase_progress(session_id: str, phase_name: str):
    """Clear phase progress when phase completes."""
    _state_manager.clear_phase_progress(session_id, phase_name)
...
def get_session_state(session_id: str) -> dict:
    ...
    Returns:
        ...
        Keys: session_id, cascade_id, status, current_phase, depth, last_update, metadata
    """
