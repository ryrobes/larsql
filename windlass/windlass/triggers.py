"""
Trigger management and export for Windlass cascades.
...
def _get_windlass_command(cascade_path: str, inputs: Optional[Dict] = None, trigger_name: str = None) -> str:
    """Generate the windlass CLI command for a trigger."""
    cmd = f"windlass run {cascade_path}"
    if inputs:
...
def export_cron(cascade: Union[str, CascadeConfig], cascade_path: str = None) -> str:
    ...
    lines = [
        f"# Windlass triggers for {config.cascade_id}",
    ...
    for trigger in config.triggers:
        ...
        if isinstance(trigger, CronTrigger):
            cmd = _get_windlass_command(cascade_path, trigger.inputs, trigger.name)
...
def export_systemd(
    ...
    service_name = service_name or f"windlass-{config.cascade_id}"
    ...
    timer_content = f"""# Windlass timer for {config.cascade_id}
# Generated at {datetime.now().isoformat()}
# Install to: /etc/systemd/system/{service_name}.timer

[Unit]
Description=Windlass trigger: {cron_trigger.name}
...
"""
    ...
    cmd = _get_windlass_command(cascade_path, cron_trigger.inputs, cron_trigger.name)

    service_content = f"""# Windlass service for {config.cascade_id}
# Generated at {datetime.now().isoformat()}
# Install to: /etc/systemd/system/{service_name}.service

[Unit]
Description=Windlass cascade: {config.cascade_id}
After=network.target

[Service]
Type=oneshot
{f'User={user}' if user else '# User=windlass'}
...
def export_kubernetes(
    ...
    for trigger in config.triggers:
        ...
        job_name = f"windlass-{config.cascade_id}-{trigger.name}".lower().replace("_", "-")
        ...
        manifest = f"""---
# Windlass CronJob for {config.cascade_id}:{trigger.name}
# Generated at {datetime.now().isoformat()}
apiVersion: batch/v1
kind: CronJob
metadata:
...
def export_airflow(
    ...
    dag_id = dag_id or f"windlass_{config.cascade_id}"
    ...
    code = f'''"""
Windlass DAG for {config.cascade_id}
Generated at {datetime.now().isoformat()}

This DAG executes the Windlass cascade via BashOperator.
"""
...
with DAG(
    ...
    description="{config.description or f'Windlass cascade: {config.cascade_id}'}",
    schedule_interval="{primary_trigger.schedule}",
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=["windlass", "{config.cascade_id}"],
) as dag:
